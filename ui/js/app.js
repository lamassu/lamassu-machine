...
function buildCassetteButtons(_cassettes, numberOfButtons) {
  var activeDenoms = _cassettes.reduce(function (acc, value) {
    acc[value.denomination] = value.count === null || value.count > 0;
    return acc;
  }, {});

  var selectedDenoms = Object.keys(activeDenoms).reduce(function (acc, value) {
    acc[value] = false;
    return acc;
  }, {});
  ...
  for (var i = 0; i < numberOfButtons; i++) {
    ...
    var intersection = Object.keys(activeDenoms).reduce(function (acc, value) {
    ...
    if (i + 1 === numberOfButtons && Object.values(intersection).every(function (it) {
      return it === false;
    })) {
      (function () {
        var firstActiveDenom = Object.keys(activeDenoms).find(function (it) {
          return activeDenoms[it] === true;
        });
        cassette = cassettes.find(function (it) {
          return it.denomination === firstActiveDenom && (it.count === null || it.count > 0);
        });
      })();
    } else {
      cassette = cassettes.find(function (it) {
        return it.denomination === cassettes[i].denomination && (it.count === null || it.count > 0);
      }) || cassettes[i] || cassettes[cassettes.length - 1];
    }
