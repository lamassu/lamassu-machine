...
var Keypad = function Keypad(keypadId, opts, callback) {
  ...
  function keyHandler(e) {
    ...
    if (target.hasClass('enter')) {
      ...
      if (self.type === 'phoneNumber') {
        var phoneResult = libphonenumber.parsePhoneNumberFromString(self.result, self.opts.country);
    ...
    if (target.hasClass('plus')) {
      return self._keyPress({ text: function text() {
          return '+';
        } });
    }
...
function isValidSsn(ssn) {
  return ssn && ssn.length === LENGTHS.usSsn && ssn.substr(0, 1) !== '9' && ssn.substr(0, 3) !== '000' && ssn.substr(0, 3) !== '666' && ssn.substr(3, 2) !== '00' && ssn.substr(5) !== '0000';
}

function customFormat(text, opts) {
  if (opts.constraint === 'date') {
    var year = text.slice(0, 4).padEnd(4, 'Y');
    var month = text.slice(4, 6).padEnd(2, 'M');
    var day = text.slice(6, 8).padEnd(2, 'D');
