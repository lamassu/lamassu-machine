...
var defaultOpts = {
  ...
  // input: '0',
  // codec: '.jpg',
  // frameCallback: function (err, frameRaw, faceDetected) {},
  // onFrame: function (err, frameRaw) {},
  // onFaceDetected: function (err, frameRaw) {},
...
exports.config = function (config) {
  if (_.get('mockCam', config) && _.get('brain.mockCameraImage', config)) {
    console.log('camera setup to use mocked camera')

    camera = require('../mocks/camera-wrapper').config(config)
  }
 ...
 * @param {(Error) => bool} [opts.onError] callback function when an error occurs
 * @param {(Error, Buffer) => bool} [opts.onFrame] callback function with the frame attached. return false to close the camera
 * @param {(Error, Buffer) => bool} [opts.onFaceDetected] callback function with the frame attached. return false to close the camera
...
function onFrameCallback (err, frameRaw, faceDetect) {
  if (_.get('onError', liveOpts) && _.isError(err)) {
    liveOpts.onError(err)
  ...
  if (faceDetect && _.get('onFaceDetected', liveOpts)) {
    if (liveOpts.onFaceDetected(err, lastFrame) === false) {
  ...
  if (_.get('onFrame', liveOpts)) {
    if (liveOpts.onFrame(err, lastFrame) === false) {
