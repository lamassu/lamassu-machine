...
module.exports = {run, register, authenticate}

function cardPresent () {
  console.log('card present')
  actionEmitter.emit('fob', {action: 'present'})

  if (_.isNil(u2fAuth)) {
    return register()
    .then(r => actionEmitter.emit('fob', {action: 'registered', record: r}))
  }

  return authenticate([u2fAuth])
  .then(r => actionEmitter.emit('fob', {action: 'authorized', record: r}))
  .catch(error => actionEmitter.emit('fob', {action: 'unauthorized', error}))
}
...
function processNfc (event) {
  switch (event.action) {
    case 'cardPresent':
      return cardPresent()
    case 'cardRemoved':
      return actionEmitter.emit('fob', {action: 'removed'})
  }
}
...
function register () {
  ...
  .then(r => writeFile(u2fPath, JSON.stringify(r)))
  .then(loadAuth)
}
