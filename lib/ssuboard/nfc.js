  ...
  return new Promise((resolve, reject) => {
    ...
    pcsc.on('reader', function (_reader) {
      ...
      reader.on('status', function (status) {
        ...
        console.log('DEBUG520: %j', status)
        console.log('DEBUG521: %j', state)
        const changes = state ^ status.state

        console.log('DEBUG501: %s, %s', status.state.toString(16), state.toString(16))

        if (changes) {
          console.log('DEBUG510')
          if ((changes & this.SCARD_STATE_EMPTY) && (status.state & this.SCARD_STATE_EMPTY)) {
            console.log('DEBUG511')
