...
let ledAddresses
let GENMEGA_MACHINE = 'genmega'
let model = deviceConfig.cryptomatModel
...
function pulseUpGM (device) {
  return lc.lightUp({ device: device, type: 'pulse' })
}

function solidUpGM (device) {
  return lc.lightUp({ device: device, type: 'solid' })
}
...
function processBrain (event) {
  switch (event.action) {
    ...
    case 'billDispenserDispensing':
      return model === GENMEGA_MACHINE ? solidUpGM('cdu') : solidUp(ledAddresses.dispenserLeds, lc.COLORS.amazonite)
    case 'billDispenserDispensed':
      return model === GENMEGA_MACHINE ? pulseUpGM('cdu') : pulseUp(ledAddresses.dispenserLeds, lc.COLORS.orange)
