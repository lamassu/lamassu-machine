...
function processBrain (event) {
  ...
  switch (event.action) {
    ...
    case 'billDispenserDispensing':
      return lc.lightUp({range: lc.LEDS.validatorLeds, color: lc.COLORS.orange, type: 'solid'})
    case 'billDispenserDispensed':
      return lc.lightUp({range: lc.LEDS.validatorLeds, color: lc.COLORS.amazonite, type: 'solid'})
