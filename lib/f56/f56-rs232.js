...
function initialize (currency, denom1, denom2, denom3, denom4) {
  const billData = bills[currency]

  if (!billData.lengths[denom1]) {
    throw new Error(`Unsupported denomination: ${denom1} for fiat code: ${currency}`)
  }

  if (!billData.lengths[denom2]) {
    throw new Error(`Unsupported denomination: ${denom2} for fiat code: ${currency}`)
  }

  if (!billData.lengths[denom3]) {
    throw new Error(`Unsupported denomination: ${denom3} for fiat code: ${currency}`)
  }

  if (!billData.lengths[denom4]) {
    throw new Error(`Unsupported denomination: ${denom4} for fiat code: ${currency}`)
  }

  const ODR = billData.polymer ? 0x40 : 0x00
  const lengths = [billData.lengths[denom1], billData.lengths[denom2], billData.lengths[denom3], billData.lengths[denom4], 0x00, 0x00]
...
function billCount (c1, c2) {
  return Promise.resolve()
    ...
    .then(res => {
      ...
      const dispensed3 = DP(res.slice(0x2b, 0x2d))
      const dispensed4 = DP(res.slice(0x2d, 0x2f))
      const rejected1 = DP(res.slice(0x2f, 0x31))
      const rejected2 = DP(res.slice(0x31, 0x33))
      const rejected3 = DP(res.slice(0x33, 0x35))
      const rejected4 = DP(res.slice(0x35, 0x37))

      const response = {
        bills: [
          {dispensed: dispensed1, rejected: rejected1},
          {dispensed: dispensed2, rejected: rejected2},
          {dispensed: dispensed3, rejected: rejected3},
          {dispensed: dispensed4, rejected: rejected4}
        ]
      }
