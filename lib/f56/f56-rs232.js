...
function initialize (currency, topDenom, bottomDenom) {
  ...
  const thicknesses = _.repeat(4, billData.thickness)
  const command = new Buffer(_.flatten([0x60, 0x02, 0x0d, ODR, lengths, thicknesses, FS]))
...
function billCount (c1, c2) {
  ...
  const command = new Buffer(_.flatten([0x60, 0x03, 0x15, ODR, billCounts, rejects, retries, FS]))
  ...
  .then(res => {
    ...
    return ({dispensed: [dispensed1, dispensed2], rejected: [rejected1, rejected2]})
  })
}

function billsPresent () {
  const command = new Buffer(_.flatten([0x00, 0x01, FS]))
