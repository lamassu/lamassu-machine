...
function initialize (currency, topDenom, bottomDenom) {
  ...
  const command = new Buffer(_.flattenDeep([0x60, 0x02, 0x0d, ODR, lengths, thicknesses, FS]))
...
function billCount (c1, c2) {
  ...
  const command = new Buffer(_.flattenDeep([0x60, 0x03, 0x15, ODR, billCounts, rejects, retries, FS]))
...
function billsPresent () {
  const command = new Buffer(_.flattenDeep([0x00, 0x01, FS]))
