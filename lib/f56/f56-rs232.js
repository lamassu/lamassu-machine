...
function initialize (currency, denominations) {
  ...
  const lengths = []
  _.forEach(it => !_.isNil(billData.lengths[denominations[it]]) ? lengths.push(billData.lengths[denominations[it]]) : lengths.push(0x00), _.times(_.identity(), 6))
...
function billCount (counts) {
  const actualCounts = _.map(it => !_.isNil(counts[it]) ? counts[it] : 0, _.times(_.identity(), 4))
