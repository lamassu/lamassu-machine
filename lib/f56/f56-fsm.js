...
const fsm = new machina.Fsm({
  ...
  states: {
    ...
    DataBody: {
      ...
      Data: byte => {
        ...
        console.log(fsm.dataPointer)
        console.log(prettyHex(fsm.data.slice(0, fsm.dataPointer - 1)))
  ...
  tx: function (packet) {
    console.log('packet: %s', prettyHex(packet))
    console.log('packet: %s', prettyHex(buildFrame(packet)))
