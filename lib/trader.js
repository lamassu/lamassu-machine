...
Trader.prototype.syncLogs = function syncLogs () {
  ...
  // Send unsaved logs to server
    ...
    .catch(err => console.log(`Sync logs error: ${err}`))
}
...
Trader.prototype.cashboxRemoved = function cashboxRemoved () {
  const rec = { uuid: uuid.v4() }

  return this.request({
    path: '/cashbox_removed',
    method: 'POST',
    body: rec,
    noRetry: false
  }).catch(() => {})
...
Trader.prototype.cashboxInserted = function cashboxInserted () {
  const rec = { uuid: uuid.v4() }

  return this.request({
    path: '/cashbox_inserted',
    method: 'POST',
    body: rec,
    noRetry: false
  }).catch(() => {})
