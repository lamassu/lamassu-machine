...
Trader.prototype.pollHandler = function pollHandler (res) {
  ...
  this.coinSettings = _.mapValues(_.mapValues(BN), res.coinSettings)

  this.balances = _.mapValues(BN, res.balances)
  this.coins = activeCoins(res)
  ...
  console.log('DEBUG103: %j', res.reboot)
...
function activeCoins (res) {
  const coins = _.keys(res.coinSettings)
