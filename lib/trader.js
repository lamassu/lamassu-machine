...
function oldIsNetworkError (err) {
  switch (err.name) {
    case 'RequestError':
    case 'ReadError':
    case 'ParseError':
...
// BACKWARDS_COMPATIBILITY 8.1.0
// Replace `this.isNetworkError` with `isNetworkError`
Trader.prototype.isNetworkError = function (err) {
  return (this.isGraphqlPollEnabled() ? isNetworkError : oldIsNetworkError)(err)
}
...
Trader.prototype.postPollEvents = function postPollEvents (res, deviceId) {
  networkDownCount = 0
  ...
  if (this.updater.isUpdating() && this.updater.isComplete()) this.trader.updateCompleted()
  if (!_.isEmpty(res.updatableMachines) && _.includes(deviceId)(res.updatableMachines)) this.emit('update')
...
Trader.prototype.pollHandler = function pollHandler (data) {
  ...
  const handleStaticConfig = data => {
    ...
    this.triggersAutomation = data.triggersAutomation
    this.triggers = data.triggers

    
    
    if (data.receiptInfo && ((this.receiptPrintingActive = data.receiptInfo.paper) || (this.smsReceiptActive = data.receiptInfo.sms))) {
  ...
  const handleTerms = terms => {
    ...
    this.terms = _.flow(
      _.pick(['delay', 'title', 'text', 'accept', 'cancel']),
      _.set('active', true)
    )(details)
  ...
  return this.postPollEvents(_.pick(['reboot', 'shutdown', 'restartServices', 'updatableMachines'], data.configs.dynamic), this.machineInfo.deviceId)
}
...
Trader.prototype.oldPollHandler = function oldPollHandler (res) {
  ...
  this.triggersAutomation = _.mapValues(_.eq('Automatic'), res.triggersAutomation)
...
Trader.prototype.handleUpdateRequest = function handleUpdateRequest () {
  try {
    if (this.updater.isUpdating()) {
      console.log('[UPDATE] Update already in progress!')
      return
    }
    this.updater._init()
    return this.updater.downloadHandler(this.request({
      path: '/update',
      method: 'GET',
      deviceId: this.machineInfo.deviceId,
      isStream: true
    }))
  } catch (err) {
...
Trader.prototype.updateCompleted = function updateCompleted () {
  return this.request({
    path: '/update/complete',
    method: 'POST',
    body: { deviceId: this.machineInfo.deviceId }
  })
    .then(res => {
      if (res.body.updateStatus === 'successful') this.updater.cleanUp()
    })
