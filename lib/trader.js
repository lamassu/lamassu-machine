...
Trader.prototype.updatePhone = function updatePhone (phone, cb) {
  this._request({
    path: '/update-phone',
    method: 'PATCH',
    body: {phone: phone}
...
Trader.prototype.fetchPhoneTx = function fetchPhoneTx (phone, cb) {
  this._request({
    path: '/phone-tx?phone=' + encodeURIComponent(phone),
    method: 'GET'
  }, function (err, res) {
    if (err) {
...
Trader.prototype.waitForDispense = function waitForDispense (status, cb) {
  this._request({
    path: '/await-dispense?not-status=' + status,
    method: 'GET'
  }, function (err, res) {
    if (err) return cb(err)
...
Trader.prototype.dispense = function dispense (originalSessionId, cb) {
  this._request({
    path: '/dispense?original-session-id=' + originalSessionId,
    method: 'GET'
  }, function (err) {
    if (err) return cb(err)
    return cb()
...
Trader.prototype._pollHandler = function _pollHandler (err, res) {
  ...
  this.balances = res.balances || {BTC: res.fiat}
  this.coins = res.coins || ['BTC']
