...
Trader.prototype.postPollEvents = function postPollEvents (res) {
  ...
  if (res.syncDeviceConfig) this.emit('syncDeviceConfig')
...
Trader.prototype.pollHandler = function pollHandler (data) {
  ...
  return this.postPollEvents(_.pick(['reboot', 'shutdown', 'restartServices', 'syncDeviceConfig'], data.configs.dynamic))
}
...
Trader.prototype.syncDeviceConfig = function syncDeviceConfig (obj) {
  return this.request({
    path: '/machine-config/sync',
    body: obj,
    method: 'POST'
  })
  .catch(err => console.error('Failed to sync device config', err))
}
