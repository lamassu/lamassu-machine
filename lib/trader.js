...
query($configVersion: Int) {
  staticConfig(currentConfigVersion: $configVersion) {
...
Trader.prototype.oldPoll = function oldPoll () {
  ...
  return this.request({
    ...
    .then(r => this.oldPollHandler(r.body))
...
Trader.prototype.graphqlPoll = function graphqlPoll () {
  return gqlClient.query({
    query: GRAPHQL_QUERY,
    variables: { configVersion: this.latestConfigVersion || null }
  })
    .then(r => this.pollHandler(r.data))
    .catch(err => this.pollError(err))
}

Trader.prototype.poll = function poll () {
  return this.isNewServer() ? this.graphqlPoll() : this.oldPoll()
}
...
Trader.prototype.isNewServer = function isNewServer () {
  return this.serverVersion && semver.gte(this.serverVersion, '8.1.0')
}
...
/*
 * TODO: Remove once the old poller is removed -- replace `this.isNetworkError`
 * with just `isNetworkError` everywhere!
 */
Trader.prototype.isNetworkError = function (err) {
  return (this.isNewServer() ? isNetworkError : oldIsNetworkError)(err)
}
...
Trader.prototype.pollHandler = function pollHandler (data) {
  ...
  const handleStaticConfig = data => {
    ...
    if (this.receiptPrintingActive = !!data.receiptInfo) {
      this.smsReceiptActive = data.receiptInfo.sms
      this.receiptOptions = _.omit(['sms'], data.receiptInfo)
    } else {
      this.smsReceiptActive = null
    ...
    this.terms = data.terms ? _.set('active', true, data.terms) : false
    this.termsConfigVersion = this.latestConfigVersion
  ...
  handleStaticConfig(data.staticConfig)
  handleDynamicConfig(data.dynamicConfig)

  this.coins = _.filter(coin => isActiveCoin(this._rates, this.balances, coin.cryptoCode), coinsRes)

  return this.postPollEvents(_.pick(['reboot', 'shutdown', 'restartServices'], data.dynamicConfig))
}
