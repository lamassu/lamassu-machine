...
Trader.prototype.isGraphqlPollEnabled = function isGraphqlPollEnabled () {
  return this.serverVersion && semver.gte(this.serverVersion, '8.1.0-beta.0')
}
...
Trader.prototype.pollHandler = function pollHandler (data) {
   ...
   *
   * Just (Just hash, Nothing)
   *   `hash` should be equal to the cached one -- meaning the T&C haven't
   *   changed.
   *
   * Just (Just hash, Just tc)
  ...
  const handleTerms = terms => {
    ...
    if (terms.hash === this.termsHash) return // T&C haven't changed

    if (!terms.details) {
      // No details even though there's a hash and it differs -- shouldn't happen!
      this.terms = false
      this.termsHash = null
      return
    }
