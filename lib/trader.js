...
Trader.prototype.waitForDispense = function waitForDispense (tx, status) {
  ...
  return new Promise((resolve, reject) => {
    const intervalHandle = setInterval(() => {
      ...
      .then(newTx => {
        console.log('DEBUG214')
        processing = false

        if (timedout) return

        clearInterval(intervalHandle)
        console.log('DEBUG215: %j', newTx)

        return resolve(newTx)
      })
      .catch(err => {
        processing = false
        console.log('DEBUG216: %s', err.stack)

        if (err.statusCode === 304) return
        console.log('DEBUG217')
...
Trader.prototype.pollHandler = function pollHandler (res) {
  console.log('DEBUG334: pollhandler')
  ...
  console.log('DEBUG787: %j', res.balances)
  this.balances = _.mapValues(BN, res.balances)
  this.coins = filterCoins(res)
  this.latestConfigVersion = res.configVersion

  console.log('DEBUG335: coins: %j', this.coins)
  console.log('DEBUG336: rates: %j', this._rates)
...
Trader.prototype.pollError = function pollError (err) {
  console.log('DEBUG334: pollhandler')
  if (err) console.log('DEBUG337: err: %s', err.stack)
