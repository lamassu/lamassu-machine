...
Trader.prototype.syncLogs = function syncLogs () {
  ...
  // Send unsaved logs to server
    .then(logs => {
      console.log(logs, 'LOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOGS')
...
Trader.prototype.machineUpdate = function machineUpdate (downloadHandler, options) {
  try {
    return downloadHandler(this.request({
      path: '/update',
      method: 'GET',
