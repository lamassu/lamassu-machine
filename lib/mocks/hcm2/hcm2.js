...
const cassetteNameByIndex = i => (['2A', '3A', '4A', '5A'][i])
  ...
  .then(body => {
    ...
    this.cassettes = cassettes.map(
      (it, idx) => Object.assign(it, {
        cassetteName: cassetteNameByIndex(idx),
...
Hcm2.prototype.dispenseByRoom = function dispenseByRoom (bills) {
  const cassettes = this.cassettes
  const params = _.flow(
    _.range(0),
    _.map(i => [`NotesRoom${cassetteNameByIndex(i)}`, bills[i]]),
    _.fromPairs,
  )(bills.length)
    ...
    case 'insertBill':
      if (this.shutterOpened) {
        const destinationUnit = _.flow(
          findCassetteByDenom(this.cassettes),
          _.defaultTo({ cassetteName: 'cashbox' }),
          _.get(['name']),
        )(denomination)
