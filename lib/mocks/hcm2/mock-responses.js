...
const cashCount = (insertedBills) => {
  state.StackedNotesEscrow = _.size(insertedBills).toString()
  state.FedNotesCashSlot = _.size(insertedBills).toString()
  ...
  return Promise.resolve(JSON.stringify({
    ...
    result: {
      ...
      StackedNotesByDenomAndDest: _.join(',', [
        ...
        _.map(it => `${it.denomination},Escrow,${_.size(_.filter(ite => ite.denomination === it.denomination, insertedBills))}`, _.uniqBy(ite => ite.denomination, insertedBills))
