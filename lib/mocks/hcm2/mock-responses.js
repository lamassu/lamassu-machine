...
const dispenseByRoom = params => {
  ...
  const FedNotes2A = _.toString(_.toNumber(params.NotesRoom2A) + 0)
  const FedNotes3A = _.toString(_.toNumber(params.NotesRoom3A) + 0)
  const FedNotes4A = _.toString(_.toNumber(params.NotesRoom4A) + 0)
  const FedNotes5A = _.toString(_.toNumber(params.NotesRoom5A) + 0)
  return Promise.resolve(JSON.parse(JSON.stringify({
    id,
    jsonrpc: '2.0',
