...
function run (cb) {
  ...
  fs.readFile('tools/fake-bills.html', (err, fakeBillsPage) => {
    if (!err) {
      const httpServer = http.createServer(httpRouter(fakeBillsPage, handler)).listen(3078)
      console.log(`Fake bills page is live on http://localhost:${httpServer.address().port}`)
    } else console.log('Error reading tools/fake-bills.html')
  });

  cb()
}

const httpRouter = (fakeBillsPage, handler) => (req, res) => {
  const safeParseInt = s => {
    try { return parseInt(s, 10)}
    catch (e) { return null }
  }

  const denomination = safeParseInt(req.url.slice(1))

  if (req.url == '/') {
    res.writeHead(200, {'Content-Type': 'text/html'});
    res.end(fakeBillsPage);
  } else if (denomination !== null) {
      res.writeHead(200, {'Content-Type': 'application/json'});
      res.end('{"message": "OK"}');
      handler({command: 'insertBill', denomination: denomination })
  } else {
