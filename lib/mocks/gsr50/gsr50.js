...
Gsr50.prototype.emitEvent = function emitEvent(bill) {
  ...
  const destinationUnit = _.find(it => it.denomination.toNumber() === bill.toNumber() && it.count < 60)(this.stackersCounts)

  if (destinationUnit) {
    this.stackersCounts[destinationUnit.number].count++
  }
...
Gsr50.prototype._setup = function _setup (data) {
  ...
  this.stackersCounts = _.keyBy(it => it.number, data.stackers)
}
