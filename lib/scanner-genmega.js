...
function scanPDF417 (callback) {
  const { iRet, data } = genmega.BarcodeScan(barcodeScannerPath)
  if (iRet < 0 && _.isEmpty(data)) {
...
function scanPairingCode (callback) {
  const { iRet, data } = genmega.BarcodeScan(barcodeScannerPath, 1)
  if (iRet < 0 && _.isEmpty(data)) {
    callback(new Error('Scan failed!'))
  }
  if (_.isEmpty(data)) return callback(null, null)
...
function scanPK (callback) {
  const { iRet, data } = genmega.BarcodeScan(barcodeScannerPath, 0)
  if (iRet < 0 && _.isEmpty(data)) {
    callback(new Error('Scan failed!'))
  }
  if (_.isEmpty(data)) return callback(null, null)
