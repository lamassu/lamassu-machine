...
function reset (basePath, platform) {
  ...
  for (var i = 0; i < dirs.length; i++) {
    ...
    try {
      ...
      console.log('DEBUG300: %s', platform)
      if (platform === 'N7G1') return binaryReset(busNum, devNum)
      if (platform === 'AAEON') return authReset(busNum, devNum)
    } catch (err) {
...
function authReset (busNum, devNum) {
  var authPath = path.resolve('/sys/bus/usb/devices/', [busNum, devNum].join('-'), 'authorized')
  console.log('DEBUG301: %s', authPath)
  try {
