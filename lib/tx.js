...
function eq (a, b) {
  return a.id === b.id &&
  a.fiat.eq(b.fiat) &&
  a.cryptoAtoms.eq(b.cryptoAtoms) &&
  a.bills.length === b.bills.length &&
  _.isMatchWith((aa, bb) => a.id === b.id, a.bills, b.bills)
}
...
function createPending (bill, exchangeRate, tx) {
  const cryptoCode = tx.cryptoCode
...
module.exports = {update, addBill, createPending, eq}
