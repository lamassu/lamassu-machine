...
function txMerger (oldValue, newValue, key, tx) {
  ...
  if (key === 'cryptoAtoms') {
    const oldValueAfterFee = oldValue.eq(0)
    ? tx.cashInFee.neg()
    : oldValue


    // Need to figure out how to calculate this correctly
    // Currently, each bill has own exchange rate
    // Should use exchange rate of first bill for computing fee in crypto
    // Don't use mergeWith, do it more manually
    return truncateCrypto(oldValueAfterFee.add(newValue), cryptoCode)
  }
...
function update (oldTx, updateTx) {
  const newTx = _.mergeWith(txMerger, oldTx, updateTx)
  const dirty = newTx.dirty || !eq(oldTx, newTx)

  return _.set('dirty', dirty, newTx)
}
