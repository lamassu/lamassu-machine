...
const solutionToOriginalUnits = (solution, units) => {
  const billsLeft = _.clone(_.fromPairs(_.map(it => [BN(it.denomination), it.provisioned])(solution)))
  return _.reduce(
    (acc, value) => {
...
const resultToUnits = (vars, cashUnits) =>
  solutionToOriginalUnits(
    vars.map(
      ([denomination, provisioned]) => ({ denomination, provisioned })
    ),
    cashUnits
  )
