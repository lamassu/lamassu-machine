...
const buildBillList = (_units, mode) => {
  const units = _.map(it => Object.assign(it, { denomination: _.isNil(it.denomination.toNumber) ? it.denomination : it.denomination.toNumber() }))(_.cloneDeep(_units))
  switch (mode) {
...
const getSolution = (units, amount, mode) => {
  ...
  return _.reduce(
    (acc, value) => {
      acc.push({ denomination: BN(value), provisioned: solution[value] })
      return acc
    },
    [],
    _.keys(solution)
...
function makeChange(outCassettes, amount) {
  ...
  if (_.isEmpty(solution)) return null
