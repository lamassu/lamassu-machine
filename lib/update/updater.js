...
Updater.prototype._fetchDeviceId = function _fetchDeviceId () {
  ...
  const firstWlanName = Object.keys(netInterfaces).filter(name => name.startsWith('wlan')).sort()[0]
  const firstEthName = Object.keys(netInterfaces).filter(name => name.startsWith('eth')).sort()[0]
  const mainInterfaceName = firstWlanName ? firstWlanName : firstEthName

  if (mainInterfaceName) {
    const macAddr = netInterfaces[mainInterfaceName][0].mac
