...
function fetchVersion() {
  var packageJson = JSON.parse(fs.readFile('/opt/machine/lamassu-machine/package.json'));
  return packageJson.version;
}

function fetchPackages() {
  try {
    var manifest = JSON.parse(fs.readFile('/opt/machine/manifest.json'));
...
Updater.prototype._init = function _init() {
  if (!fs.existsSync(this.config.certs.keyFile) || !fs.existsSync(this.config.certs.certFile))
    return false;
...
var certs = deviceConfig.brain.certs;
var config = softwareConfig.updater;
config.certs = certs;
_.merge(config, deviceConfig.updater);
