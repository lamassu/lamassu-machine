...
Updater.prototype._fetchDeviceId = function _fetchDeviceId() {
  return fs.readFileSync('/sys/class/net/wlan0/address', {encoding: 'utf8'}).trim().replace(/:/g,'-');
};
...
Updater.prototype._init = function _init() {
  if (!fs.existsSync(this.config.keyFile) || !fs.existsSync(this.config.certFile))
    return false;
  ...
  if (fs.existsSync(versionFile))
    this.version = JSON.parse(fs.readFileSync(versionFile)).version;
  else
    this.version = 1;
...
Updater.prototype._download = function _download() {
  if (this.downloading) return;
  var self = this;
  this.httpsOptions.headers['application-version'] = this.version;
  this.httpsOptions.headers['device-id'] = this.deviceId;
...
*/
  ...
  res.on('error', function(err) { this.downloading = false; self.emit('error', err); });
};
...
Updater.prototype._updateDownloadInfo = function _updateDownloadInfo() {
  var versionFile = this.config.downloadDir + '/download.json';
  var timestamp = new Date();
  var json = JSON.stringify({version: this.version, timestamp: timestamp});
...
var config = softwareConfig.updater;
