...
var Updater = function (config) {
  ...
  if (this.deviceId == null) {
    console.log('[WARN]: Device ID could not be retrived.')
  }
}
...
Updater.prototype._fetchDeviceId = function _fetchDeviceId () {
  ...
  let macAddrs = []
  for (let [key, val] of Object.entries(netInterfaces)) {
    val.forEach(item => {
      if (item.mac !== '00:00:00:00:00:00') {
          macAddrs.push({ name: [key], mac: item.mac })
      }
    })
  }

  macAddrs.sort((lhs, rhs) => {
      if (lhs.mac < rhs.mac) return -1
      else if (lhs.mac > rhs.mac) return 1
      else return 0
  })

  if (macAddrs.length > 0) {
    const name = macAddrs[0].name
    const mac = macAddrs[0].mac.replace(/:/g, '-')
    console.log(`[INFO]: Reporting the MAC address of interface ${name} as the Device ID: ${mac}`)
    return mac
  }
  else return null
}
