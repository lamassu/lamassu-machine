...
const dispenseGenerator = function* dispenseGenerator(notes, tx, txId) {
  ...
  console.log('DISPENSE | notes', notes);

  if (notes.length > 1) emit({ action: 'toggleBatchCounter', state: 'show' })
  ...
  if (!dispenseConfirmed) {
    emit({ action: 'transitionState', state: 'outOfCash' })
    throw new Error('dispense error5')
  }
  ...
  emit({ action: 'toggleBatchCounter', state: 'hide' })
}
