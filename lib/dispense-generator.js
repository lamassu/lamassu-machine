...
const dispenseGenerator = function * dispenseGenerator (notes, tx, timeout) {
  let result = []
  let error = {}

  for (let index = 0; index < notes.length; index++) {
    emit({ action: 'updateUI', current: index + 1, of: notes.length })
    let yielded = yield emit({ action: 'dispenseBatch', notes: notes[index], current: index + 1, of: notes.length })
    if (yielded !== undefined) {
      console.log(`Dispensed amount: ${yielded.value}`)
      if (yielded.value) result.push(yielded.value)
  ...
  const fastUpdateTxEventDataErr = {
    error: _.join(' ', _.reject(_.isEmpty, [error.name, error.message, error.err, error.error, error.statusCode]))
  }
  ...
  pDelay(timeout).then(() => {
    ...
    emit({ action: 'completed', txId: tx.id })
  })
}
...
module.exports = function (notes, tx, timeout) {
  let g = dispenseGenerator(notes, tx, timeout)
  g.next()
  ...
  if (notes.length > 1) {
    actionEmitter.on('billDispenser', event => {
      if (event.action === 'failure') {
        ...
        emit({ action: 'billDispenserPartialDispensed', value: event.value, current: event.current, of: event.of })
      }
    })
