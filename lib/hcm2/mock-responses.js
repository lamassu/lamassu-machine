  ...
  return JSON.stringify({
    ...
    result: {
      ...
      StackedNotesByDenomAndDest: _.join(',', [
        ...
        _.map(it => `${it},Escrow,${_.size(_.filter(ite => ite.denomination === it.denomination, params.bills))}`, _.uniq(params.bills))
...
const deposit = (id, params) => {
  ...
  const cassette2ABills = _.filter(it => it.denomination.toString() === _.find(ite => ite.cassetteName === '2A', params.cassettes).denomination.toString(), params.bills)
  const cassette3ABills = _.filter(it => it.denomination.toString() === _.find(ite => ite.cassetteName === '3A', params.cassettes).denomination.toString(), params.bills)
  const cassette4ABills = _.filter(it => it.denomination.toString() === _.find(ite => ite.cassetteName === '4A', params.cassettes).denomination.toString(), params.bills)
  const cassette5ABills = _.filter(it => it.denomination.toString() === _.find(ite => ite.cassetteName === '5A', params.cassettes).denomination.toString(), params.bills)
  ...
  return JSON.stringify({
    ...
    result: {
      ...
      FedNotes4A: state.FedNotes4A,
      FedNotes5A: state.FedNotes5A,
      StackedNotesByDenomAndDest: _.join(',', _.map(it => `${it},1A,${_.size(_.filter(ite => ite.denomination === it.denomination, params.bills))}`, _.uniq(params.bills))), // '7,1A,2,1,2A,1,3,3A,1,5,5A,1'
