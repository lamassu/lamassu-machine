...
Hcm2.prototype.registerUSB = function registerUSB () {
  return got.post(
    'http://localhost:5000/api/ac/configure/registerUSB.cgi',
    {
      json: true,
      body: {
        jsonrpc: '2.0',
  ...
  ).then(({ body }) => _.isNil(body.error) ? body : Promise.reject(new Error(body)))
}
...
Hcm2.prototype.cashCount = function cashCount () {
  return got.post(
    'http://localhost:5000/api/ac/hitachi/cashCount.cgi',
    {
      json: true,
      body: {
        jsonrpc: '2.0',
...
function getError (action, res) {
  ...
  const errorFields = _.pick([
      ...
      'commonErrorCode',
    ].concat(cassetteErrorFields),
    res.result)

  if (_.any(it => it === 'true', _.pick(cassetteErrorFields, errorFields)) || errorFields.error !== 'OK')
