...
Hcm2.prototype.setInfo = function setInfo () {
  return rpc_call("ac/hitachi/setInfo.cgi", {
    ...
    params: {
      ...
      DenomCassette2A: _.get([0, 'noteId'], this.recyclers), // '1', // list of denoms with COMMA separator (max.16 values: 0-128)
      DenomCassette3A: _.get([1, 'noteId'], this.recyclers), // '3', // list of denoms with COMMA separator (max.16 values: 0-128)
      DenomCassette4A: _.get([2, 'noteId'], this.recyclers), // '4', // list of denoms with COMMA separator (max.16 values: 0-128)
      DenomCassette5A: _.get([3, 'noteId'], this.recyclers), // '5', // list of denoms with COMMA separator (max.16 values: 0-128)
...
Hcm2.prototype.dispenseByRoom = function dispenseByRoom (bills) {
  ...
  .then(result => { // Passthrough potential errors since they'll be handled in a parent function
    if (result.error === 'OK') return result
    // Note shortage
    if (result.commonErrorCode === '93' && result.commonErrorCodeDetail === '010000') return result
    // TODO: bills rejected
    if (false) return this.retractEscrow().catch(console.log).then(() => result)
...
Hcm2.prototype.dispense = function dispense (notes) {
  return this.dispenseByRoom(notes)
    .then(res => ({
      ...
      value: _.map(
        f => ({ dispensed: _.toNumber(_.get([f], res)), rejected: 0 }),
        ['DispenseCountRoom2A', 'DispenseCountRoom3A', 'DispenseCountRoom4A', 'DispenseCountRoom5A']
      ),
    }))
    .then(res => this.openShutter().then(() => res))
    // TODO(siiky):
    // error: 'Error:1907',
    // responseCode: '07',
