...
const lsRecyclerNameByEnvoyName = {
  '2A': 'recycler1',
  '3A': 'recycler2',
  '4A': 'recycler3',
  '5A': 'recycler4',
...
Hcm2.prototype.setInfo = function setInfo () {
  const noteIDByRecyclerIndex = idx => _.flow(
    _.find(({ name }) => name === recyclerNameByIndex(idx)),
    _.get(['noteId']),
    _.defaultTo(Promise.reject(`recycler${idx} has no configured denomination`)),
  )(this.recyclers)

  return Promise.all(_.map(noteIDByRecyclerIndex, _.range(0, 4)))
    .then(([DenomCassette2A, DenomCassette3A, DenomCassette4A, DenomCassette5A]) =>
      rpc_call("ac/hitachi/setInfo.cgi", {
        method: 'setInfo',
        ...
        params: {
          ...
          DenomCassette2A, // '1', // list of denoms with COMMA separator (max.16 values: 0-128)
          DenomCassette3A, // '3', // list of denoms with COMMA separator (max.16 values: 0-128)
          DenomCassette4A, // '4', // list of denoms with COMMA separator (max.16 values: 0-128)
          DenomCassette5A, // '5', // list of denoms with COMMA separator (max.16 values: 0-128)
...
Hcm2.prototype.reset = function reset (mode) {
  ...
  .then(reply => {
    this.leftoverBillsInCashSlot = reply.responseCode === '02' && reply.commonErrorCode === '81'
    if (!this.leftoverBillsInCashSlot) bodyOrThrow('reset', reply)
  })
}
...
Hcm2.prototype.cashCount = function cashCount () {
  ...
  .then(({ reply }) =>
    reply.commonErrorCode === '84' && reply.commonErrorCodeDetail === '030000' ?
      null :
...
Hcm2.prototype.startUp = function startUp (recyclers) {
...
Hcm2.prototype.dispense = function dispense (notes) {
  return this.dispenseByRoom(notes)
    .then(res => ({
      ...
      value: _.map(
        f => ({ dispensed: _.toNumber(_.get(['result', f], res)), rejected: 0 }),
...
Hcm2.prototype.cashSlotBillsRemoved = function () {
  return this.closeShutter()
    ...
    .then(({ shutterAreaHasBanknotes, cashSlotHasBanknotes }) => {
      if (shutterAreaHasBanknotes || cashSlotHasBanknotes) return this.openShutter()
...
Hcm2.prototype.leftoverBillsRemoved = function () {
  return this.closeShutter()
    .then(() => this.status())
    .then(({ shutterAreaHasBanknotes, cashSlotHasBanknotes }) => {
      this.leftoverBillsInCashSlot = shutterAreaHasBanknotes || cashSlotHasBanknotes
      if (this.leftoverBillsInCashSlot) return this.openShutter()
    })
}
...
Hcm2.prototype.canSendCoins = function () {
  return this.closeShutter()
    ...
    .then(canSendCoins =>
      canSendCoins || this.openShutter().then(() => false).catch(() => false)
    )
}
