...
const any = (p, a) => !_.every(_.negate(p), a)
...
Hcm2.prototype.getInfo = function getInfo () {
  const defaultDenomCodeSettings = {
    USD: [
  ...
  .then(reply => {
    this.denomCodeSettings = defaultDenomCodeSettings[this.fiatCode] ?? _.flow(
...
Hcm2.prototype.dispense = function dispense (bills) {
  ...
  const shouldOpenShutter = ({ value }) =>
    any(({ dispensed }) => dispensed > 0, value)
