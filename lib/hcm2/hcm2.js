...
const findCassetteByDenom = cassettes => denom =>
  _.find(({ denomination }) => denomination.eq(denom), cassettes)

const predictBillsDestinations = (cassettes, bills) => {
  const capacities = {
    'cassette1': 2800,
    'cassette2': 2800,
    'cassette3': 2800,
  ...
  const predictBillDestination = (cassettes, bill) => _.flow(
    _.get(['denomination']),
    findCassetteByDenom(
      _.filter(({ count, name }) => count < capacities[name], cassettes)
    ),
  ...
  return _.map(
    bill => {
      const destinationUnit = predictBillDestination(cassettes, bill)
...
Hcm2.prototype.reset = function reset (mode) {
  ...
  .then(reply =>
    reply.responseCode === '02' && reply.commonErrorCode === '81' /*&& reply.commonErrorCodeDetail === '01015C'*/ ? /* Notes remain in the cash slot. */
      this.emit('startUpRemoveBills') :
...
Hcm2.prototype.cashCount = function cashCount () {
  ...
  .then(({ reply, result, rejected }) => {
    ...
    const accepted = predictBillsDestinations(this.cassettes, result.Escrow)
    this.acceptedPending = []
...
Hcm2.prototype.startUpBillsRemoved = function () {
  return this.closeShutter()
    .then(() => this.status())
    .then(({ shutterAreaHasBanknotes, cashSlotHasBanknotes }) =>
      (shutterAreaHasBanknotes || cashSlotHasBanknotes) ?
        this.openShutter().then(() => false).catch(() => false) :
        true
    })
}
