    ...
    makeURL("ac/hitachi/setInfo.cgi"),
    {
      ...
      body: {
        ...
        params: {
          ...
          DenomCassette2A: getNoteIDByCassetteName('2A', '4'), // '1', // list of denoms with COMMA separator (max.16 values: 0-128)
...
Hcm2.prototype.cashCount = function cashCount () {
  return got.post(
    ...
    .then(_.flow(
      ...
      _.split(','),
      _.chunk(3),
      // TODO: Do we really only care about bills in Escrow?
      _.filter(([noteId, destination, count]) => destination === 'Escrow'),
      _.flatMap(([noteId, destination, count]) => repeat(count, { denomination: this.denominations[noteId] }))
    ))
}
