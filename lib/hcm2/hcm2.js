...
Hcm2.prototype.reset = function reset (mode) {
  ...
  .then(reply =>
    reply.responseCode === '02' && reply.commonErrorCode === '81' /*&& reply.commonErrorCodeDetail === '01015C'*/ ? /* Notes remain in the cash slot. */
      this.retractEscrow().catch(console.log).then(() => reply) :
...
Hcm2.prototype.cashCount = function cashCount () {
  ...
  .then(({ reply, result, rejected }) => {
    ...
    const accepted = _.map(
      ({ denomination, destination }) => ({ denomination, destination: findCassetteByDenom(denomination) }),
      result.Escrow
    )
    this.acceptedPending = []
  ...
  .then(({ reply }) =>
    reply.commonErrorCode === '84' && reply.commonErrorCodeDetail === '030000' ?
      Promise.resolve({ reply, result: { Escrow: [] } }) :
