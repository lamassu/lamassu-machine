...
WiFi.prototype.startScanning = function startScanning () {
  ...
  self._scan()
  this.scanTimer =
    setInterval(function () { self._scan() }, consts.scanInterval)
}
...
WiFi.prototype.waitConnection = function waitConnection (cb) {
  ...
  async.until(
    function () {
      ...
      var timeout = elapsed > consts.connectionTimeout
    ...
    function (lcb) {
      self.status(function (err, res, localIp) {
        ...
        if (connectionStatus === 'connected') {
          ip = localIp
          lcb(null)
        } else setTimeout(lcb, consts.checkInterval)
      })
    },
    function (err) {
      if (connectionStatus !== 'connected') {
...
WiFi.prototype.displaySsid = function displaySsid (ssid) {
  return this._truncate(ssid, consts.truncateLength)
}
...
WiFi.prototype._strength = function _strength (signal) {
  ...
  var maxSignal = consts.maxSignal
  var minSignal = consts.minSignal
