...
const { cdu, return_codes } = require('genmega')
...
BillDispenser.prototype._status = function _status () {
  const { iRet, result: statuses } = cdu.status()
  return {
...
BillDispenser.prototype._dispense = function (notes) {
  const { iRet, result } = cdu.dispense(notes, this.numberOfCassettes)
  return {
...
BillDispenser.prototype._present = function () {
  return this._makeError(cdu.present().iRet)
}

BillDispenser.prototype._retract = function () {
  return this._makeError(cdu.retract().iRet)
}
...
BillDispenser.prototype.dispense = function dispense (notes, _currentBatch, _batchAmount) {
  ...
  const asRecycler = resolve => {
    ...
    const timeout = setTimeout(() => {
      ...
      const { iRet: iRetShutter } = cdu.shutterAction(SHUTTER_ACTION.CLOSE)
...
BillDispenser.prototype._verifyLicenseKey = function _verifyLicenseKey (license) {
  this._throwMakeError(cdu.verifyLicenseKey(license).iRet)
}
...
BillDispenser.prototype._openSerialPort = function _openSerialPort (serialPortName) {
  ...
  this._throwMakeError(cdu.open(serialPortName).iRet)
}

BillDispenser.prototype._resetDevice = function _resetDevice (statuses) {
  if (this._isDispenser())
    return this._throwMakeError(cdu.reset(INIT_MODE.NORMAL).iRet)
  ...
  if (this._isRecycler()) {
    ...
    if (statuses.iStackerRemain === '1' || statuses.iTransporterRemain === '1') {
      this._throwMakeError(cdu.forceEject().iRet) // move the detected notes in stacker into eject-ready position
      return this._throwMakeError(cdu.reset(INIT_MODE.FORCED).iRet) // reject the notes to reject Bin if the notes are on the feeding path
    }

    if (statuses.iShutterStatus === '1') {
      this._throwMakeError(cdu.shutterAction(SHUTTER_ACTION.CLOSE).iRet)
    }
...
BillDispenser.prototype._setCassetteNumber = function _setCassetteNumber () {
  ...
  this._throwMakeError(cdu.setCassetteNumber(this.numberOfCassettes).iRet)
}
