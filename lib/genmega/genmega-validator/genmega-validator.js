...
GenMegaValidator.prototype._run = function _run (cb) {
  ...
  try {
    ...
    this._open(this.device, this.escrowEnabled)
    this._connect()
    this._poll()
...
GenMegaValidator.prototype._open = function (serialPortName, escrowEnabled) {
  if (!serialPortName) throw new Error('No serial port name provided!')
  let { iRet } = genmega.BAUOpen(serialPortName)
  if (iRet < 0) {
    this.emit('error', returnValuesTable[iRet])
  }

  if (typeof(escrowEnabled) === 'boolean') {
    iRet = genmega.BAUSetCapabilities(escrowEnabled).iRet
    if (iRet < 0) {
      this.emit('error', returnValuesTable[iRet])
    }
  }
