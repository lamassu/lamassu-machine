...
Gsr50.prototype.connect = function connect (currentRetry = 0) {
  ...
  return got.post(`http://${GSR_SERVER}:9000/connect`, {
  timeout: {
		lookup: 100,
		connect: 50,
		secureConnect: 50,
...
Gsr50.prototype.reject = function reject () {
  ...
  return this.cashRollback().then(() => this.cashCount())
}
...
Gsr50.prototype.dispense = function dispense (notes) {
  ...
  return this.getDeviceCashUnits()
    ...
    .then(([pre, r]) => Promise.all([pre, resolveOrThrow(r, '/dispense')]))
    .then(([pre, r]) => Promise.all([pre, r, this.getDeviceCashUnits()]))
    .then(([preCashUnits, [_CashUnits, billsDispensed], postCashUnits]) => {
      let CashUnits
      let hasError = !_CashUnits?.length && !!billsDispensed?.length
      if (hasError) {
      ...
      _.forEach(it => {
        ...
        if (!unit) return
        console.log(it.Position, it.DispensedCount, it.RejectCount)
        console.log(unit.dispensed, unit.rejected)
        unit.dispensed += it.DispensedCount
        unit.rejected += it.RejectCount
      })(CashUnits)

      console.log(hasError)
      console.log(hasError ? 'Bad counts on the recycler' : null)
