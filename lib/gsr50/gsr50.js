...
Gsr50.prototype.init = function init (data) {
  ...
  return new Promise(resolve => {
    ...
    setTimeout(function () {
      ...
      return self.connect()
        .then(() => self.getDeviceState())
        .then(() => self.getDeviceCashUnits())
        .then(() => resolve())
    }, 1000)
  })
}
  ...
  return got.post('http://localhost:9000/emptyUnit')
    ...
    .then((_units = []) => {
      const units = _.reduce(
        (acc, value) => Object.assign(acc, { [_.toLower(_.invert(CASH_UNIT_POSITIONS)[value.Position])]: value.Count }),
        {},
        _units
      )
...
Gsr50.prototype.updateCounts = function updateCounts (newCounts) {
  _.forEach(it => it.count = newCounts[it.name])(this.cassettes)
  _.forEach(it => it.count = newCounts[it.name])(this.stackers)
