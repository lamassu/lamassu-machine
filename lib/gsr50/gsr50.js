...
Gsr50.prototype.cashCount = function cashCount () {
  ...
  return got.post('http://localhost:9000/acceptCashStart')
    ...
    .then(res => {
      ...
      //   (acc, value) => {
      //     acc.push(..._.map(it => {
      //       const destinationUnitName = _.toLower(_.invert(CASH_UNIT_POSITIONS)[it.Destination])
      //       const destinationUnit = _.find(ite => ite.name === destinationUnitName)(_stackers)
      //       if (!_.isNil(destinationUnit)) destinationUnit.count++
      //       return { denomination: it.Denomination, destinationUnit: destinationUnitName }
      //     })(value.Notes))
      //     return acc
      //   },
      //   [],
