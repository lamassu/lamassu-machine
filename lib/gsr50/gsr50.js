...
Gsr50.prototype.run = function run (cb, { cassettes, recyclers }) {
  ...
  return this.connect()
    ...
    .then(() => this.setDeviceCashUnits(cassettes, recyclers))
    .then(() => this.reset('Mechanical'))
    .then(() => cb())
    .catch(err => cb(err))
}
...
Gsr50.prototype.cashCount = function cashCount () {
  ...
  return this.getDeviceCashUnits()
    .then(it => {
      const escrow = _.find(unit => unit.Position === CASH_UNIT_POSITIONS.recycler1)(it)
      escrowCount = escrow?.Count ?? 0
      escrowDenom = escrow?.Currency?.Denomination
    })
    .then(() => got.post(`http://${GSR_SERVER}:9000/acceptCashStart`))
    ...
    .then(res => {
      ...
      if (!_.isNil(AcceptedBills)) {
        AcceptedBills.forEach(ab => {
          ...
          if (destinationUnit === 'recycler1') {
            const isSameDenomAsEscrow = ab.Denomination === escrowDenom
            if (isSameDenomAsEscrow && escrowCount < 20) {
              escrowCount++
            } else {
              destinationUnit = 'cashbox'
            }
          }
...
Gsr50.prototype._setup = function _setup (data) {
  ...
  this.originalRecyclers = data.originalRecyclers
}
...
Gsr50.prototype.dispense = function dispense (notes) {
  const billsToDispense = _.map.convert({cap: false})((it, idx) => ({ Denomination: it.denomination.toNumber(), Amount: notes[idx], FiatCode: this.fiatCode }), _.concat(this.originalCassettes, this.originalRecyclers))
  ...
  return this.getDeviceCashUnits()
    ...
    .then(([preCashUnits, r, [_CashUnits, billsDispensed], postCashUnits]) => {
      ...
      const units = _.map(it => ({ name: it.name, dispensed: 0, rejected: 0 }))(_.concat(this.originalCassettes, this.originalRecyclers))
...
Gsr50.prototype.emptyUnit = function emptyUnit () {
  return got.post(`http://${GSR_SERVER}:9000/emptyUnit`)
    .then(r => {
      const res = JSON.parse(r.body)
      if (!res.Success) throw new Error('Error emptying units')
    })
...
Gsr50.prototype.updateCounts = function updateCounts (newCounts) {
  ...
  _.forEach(it => it.count = _.defaultTo(it.count, newCounts[it.name]))(this.recyclers)

  return this.setDeviceCashUnits(this.cassettes, this.recyclers)
}
