...
Gsr50.prototype.cashCount = function cashCount () {
  ...
  return this.getDeviceCashUnits()
    .then(it => {
      const escrow = _.find(unit => unit.Position === CASH_UNIT_POSITIONS.stacker1f)(it)
...
Gsr50.prototype.dispense = function dispense (notes) {
  ...
  return this.getDeviceCashUnits()
    ...
    .then(([preCashUnits, r, [_CashUnits, billsDispensed], postCashUnits]) => {
      ...
      const units = _.map(it => ({ name: it.name, dispensed: 0, rejected: 0 }))(_.concat(this.originalCassettes, this.originalStackers))
