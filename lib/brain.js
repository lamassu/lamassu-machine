...
Brain.prototype._periodicLog = function _periodicLog () {
  ...
  function readFile (filePath, cb) {
    return fs.readFile(filePath, {encoding: 'utf8'}, function (err, res) {
      if (err) return cb()
      return cb(null, res)
    })
  }
  ...
  if (tempSensorPath) {
    tasks.temperature = async.apply(async.map, tempSensorPath, readFile)
  }
  ...
  function reporting () {
    ...
    async.parallel(tasks, function (err, results) {
      ...
      if (results.temperature) {
        var temperature = results.temperature.filter(function (t) { return t })[0]
        if (temperature) {
          clauses.push('CPU temperature: ' +
            (temperature.trim() / 1000) + '° C')
        }
      }
