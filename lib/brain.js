...
Brain.prototype._periodicLog = function _periodicLog() {
  ...
  function reporting() {
    ...
    if (batteryCapacityPath) {
      fs.readFile(batteryCapacityPath, {encoding: 'utf8'}, function (err, batteryCapacity) {
        console.log('memUse: %s, memFree: %s, nodeUptime: %s, osUptime: %s, battery: %s', 
          memUse, memFree, nodeUptime, osUptime, batteryCapacity + '%');
      });
    } else {
      console.log('memUse: %s, memFree: %s, nodeUptime: %s, osUptime: %s', 
        memUse, memFree, nodeUptime, osUptime);
    }
  }
  reporting();
...
Brain.prototype._start = function _start() {
  if (this.trader.idVerificationLimit <= this.billValidator.lowestBill()) {
    this._startIdScan();
  } else {
...
Brain.prototype._idCode = function _idCode(code) {
  this._transitionState('verifyingId');
  this.idVerify.addLicenseCode(code);
  var self = this;
  this.idVerify.verify(function (err, result) {
    if (err) throw err;
    if (result.success) {
      // TODO COMPLIANCE: add error screen
      self._startAddressScan();
    }
  });
};
