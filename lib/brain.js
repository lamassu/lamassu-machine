...
const caPath = path.resolve(dataPath, 'ca.pem')
...
Brain.prototype._connect = function _connect () {
  ...
  if (this.clientCert) return this._startTrading()
...
Brain.prototype._pair = function _pair (totem) {
  ...
  pairing.pair(totem, this.clientCert, connectionInfoPath, function (err, connectionInfo) {
    if (err) return self._pairingError(err)
    self._idle()
  })
}
...
Brain.prototype.initTrader = function initTrader () {
  ...
  const ca = pairing.getCa(caPath)

  if ((!connectionInfo || !ca) && !this.trader.isMock) return this._unpaired()

  const config = this.rootConfig
  const protocol = config.http ? 'http:' : 'https:'

  if (config.mockTrader) {
    this.trader = require('./mocks/trader')(protocol, this.clientCert, connectionInfo, ca)
  } else {
    this.trader = require('./trader')(protocol, this.clientCert, connectionInfo, ca)
  }
