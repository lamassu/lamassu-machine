...
Brain.prototype.acceptTerms = function () {
  const updateRec = {
    termsAccepted: true
  }
...
Brain.prototype._start = function _start () {
  ...
  if (!this.tx.termsAccepted && !this.trader.terms.active) {
    // terms are automatically accepted
    // if terms screen is not enabled
    updateRec.termsAccepted = true
  }
  ...
  if (!this.tx.termsAccepted) {
    this._showTermsScreen()
  } else {
...
Brain.prototype._showTermsScreen = function _startAddressScan () {
  this._transitionState('termsScreen')

  // after 60 sec return to main screen
  this.screenTimeout = setTimeout(() => {
    if (this.state !== 'termsScreen') return
    this._idle() // TODO: check why is not showing start screen
