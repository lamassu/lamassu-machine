...
const suspend = require('./compliance/flows/suspend')
const block = require('./compliance/flows/block')
const { getAmountToHardLimit, getTriggered } = require('./compliance/triggers/triggers')
const { ORDERED_REQUIREMENTS } = require('./compliance/triggers/consts')
...
Brain.prototype.triggerSuspend = function triggerSuspend () {
  ...
  return this.trader.triggerSuspend(customer.id)
    .then(result => {
      this.customer = result.customer
    })
    .catch(err => {
...
Brain.prototype.smsFlowHandleReturnState = function smsFlowHandleReturnState () {
  ...
  // TODO new-admin - if it's start of tx and customer is suspended

  // const isStartOfTx = BN(0).eq(this.tx.fiat)
  // if (isStartOfTx && dailyLimitReached(this.trader, amount)) {
