...
Brain.prototype.processPending = function processPending () {
  return pify(fs.readFile)(this.dbFile)
  .then(f => {
    const txs = _.map(JSON.parse, f.trim().split('\n'))
    return Promise.all(_.map(r => this.processEachPending(r), txs))
  })
}

Brain.prototype.processEachPending = function processEachPending (tx) {
...
Brain.prototype._start = function _start () {
  ...
  console.log('DEBUG222: %j', {fiatBalance, highestBill})
  if (highestBill.lte(0)) return this._balanceLow()
...
Brain.prototype._networkDown = function _networkDown () {
  ...
  if (!this.tx || !this.tx.direction || !this.tx.direction === 'cashIn') {
    return this._forceNetworkDown()
  }

  if (this.tx.bills.length === 0) return this._forceNetworkDown()
...
Brain.prototype.updateTx = function updateTx (updateTx, _tx) {
  const tx = _tx || this.tx

  const oldTx = tx
  const newTx = Tx.update(oldTx, updateTx)

  if (_tx) this.tx = newTx

  return this.saveTx(newTx)
  .then(() => this.trader.postTx(newTx))
  .then(serverTx => {
    this.tx = serverTx
    return tx
  })
...
Brain.prototype.updateSpecificTx = function updateSpecificTx (updateTx) {
  console.log('DEBUG920')
  const oldTx = this.tx
  this.tx = Tx.update(oldTx, updateTx)

  console.log('DEBUG921')
...
Brain.prototype.sendTx = function sendTx (tx) {
  const postTx = this.trader.postTx(this.tx)
  .then(tx => {
    this.saveTx(this.tx)
  ...
  const $postTx = Rx.Observable.fromPromise(postTx)
...
Brain.prototype._executeSendCoins = function _executeSendCoins () {
  ...
  const postTx = () => this.updateTx({send: true})
  .then(tx => ({tx}))
  .catch(err => ({err}))
  .then(_.tap(console.log))

  const $timeout = Rx.Observable.timer(30000)
  .mapTo({timeout: true})
  ...
  .takeWhile(r => !r.tx)
  .map(r => r.tx)
  .combineLatest($timeout.startWith({}), _.merge)

  $timeout.subscribe(() => this._sendCoinsError(new Error('timeout')))

  return $source.subscribe(r => {
    if (r.tx && !r.timeoutState) return this._cashInComplete(r.tx)
  })
}
...
Brain.prototype._cashOut = function _cashOut () {
  ...
  .then(ntx => {
    console.dir(ntx)
    console.log('DEBUG24: %j', ntx)
    const amountStr = this.toCryptoUnits(ntx.cryptoAtoms, ntx.cryptoCode).toString()
    const depositUrl = coinUtil.depositUrl(ntx.toAddress, ntx.cryptoCode, amountStr)
...
Brain.prototype._physicalDispense = function _physicalDispense () {
  ...
  this.billDispenser.dispense(notes, function (err, result) {
    ...
    tx.error = result.err
    self.fastUpdateTx({bills: tx.bills, error: tx.error})
