...
const Brain = function (config) {
  ...
  this.manualTriggersValidator = _.zipObject(AUTOMATABLE_REQUIREMENTS, Array(AUTOMATABLE_REQUIREMENTS.length).fill(false))
  this.pingInterval = null
  this.speedtestInterval = null

  pingInterval = setInterval(() => {
    ping.pingRepository()
      .then(this.trader.networkHeartbeat)
  }, this.config.pingInterval)

  speedtestInterval = setInterval(() => {
    ping.checkDownloadSpeed(10)
      .then(this.trader.networkPerformance)
...
Brain.prototype.scanPDF = function scanPDF () {
  ...
  this.scanner.scanPDF417((err, result) => {
    ...
    if (_.get('idCardData')(this.trader.triggersAutomation) === MANUAL) {
      this.manualTriggersValidator.idCardData = true
    }
...
Brain.prototype.saveUsSsn = function saveUsSsn () {
  ...
  return this.trader.updateCustomer(customer.id, { usSsn: usSsn.getData() }, this.tx.id)
    .then(result => {
      this.customer = result.customer
      if (_.get('usSsn')(this.trader.triggersAutomation) === MANUAL) {
        this.manualTriggersValidator.usSsn = true
      }
...
Brain.prototype.scanPhotoCard = function scanPhotoCard () {
  ...
  this.scanner.scanPhotoCard((err, result) => {
    ...
    if (_.get('idCardPhoto')(this.trader.triggersAutomation) === MANUAL) {
      this.manualTriggersValidator.idCardPhoto = true
    }
...
Brain.prototype.takeFacephoto = function takeFacephoto () {
  this.scanner.takeFacephoto((err, result) => {
    ...
    if (_.get('facephoto')(this.trader.triggersAutomation) === MANUAL) {
      this.manualTriggersValidator.facephoto = true
    }
...
Brain.prototype.smsFlowHandleReturnState = function smsFlowHandleReturnState () {
  ...
  // const nonCompliantManualTiers = _.filter(tier => _.get(tier)(this.trader.triggersAutomation) === MANUAL)(nonCompliantTiers)
...
Brain.prototype._chooseCoin = function _chooseCoin (data) {
  this.manualTriggersValidator = _.mapValues(this.manualTriggersValidator, () => false)
