...
Brain.prototype._chooseCoin = function _chooseCoin (cryptoCode) {
  return this.updateTx({cryptoCode})
  .then(() => {
...
Brain.prototype.updateTx = function updateTx (updateTx) {
  const oldTx = this.tx
  this.tx = Tx.update(updateTx, oldTx)

  if (Tx.eq(this.tx, oldTx)) return Promise.resolve(newTx)
...
Brain.prototype._uiCredit = function _uiCredit () {
  const tx = Tx.addBill(this.pending, this.tx)
...
Brain.prototype._createPendingTransaction = function _createPendingTransaction (bill) {
  console.assert(!this.pending || this.pending.fiat.eq(0), "pending fiat is positive, can't start tx")
  const cryptoCode = this.tx.cryptoCode
  const exchangeRate = this.trader.rates(cryptoCode).cashIn

  return Tx.createPending(bill, exchangeRate, this.tx)
}
...
Brain.prototype._fiatButton = function _fiatButton (data) {
  ...
  const unitScale = new BigNumber(10).pow(Tx.coins[cryptoCode].unitScale)
  ...
  if (buttons.activeDenominations.activeMap[denomination]) {
    ...
    tx.cryptoAtoms = Tx.truncateCrypto(tx.cryptoAtoms.plus(cryptoAtoms), cryptoCode)
  }
