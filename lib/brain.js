...
Brain.prototype._idle = function _idle (locale) {
  ...
  this.setDispenseWaitEnded(false)
...
Brain.prototype.setDispenseWaitEnded = function setDispenseWaitEnded (ended) {
  return this.dispenseWaitEnded = ended
}

Brain.prototype._waitForDispense = function _waitForDispense (status) {
  const originalTx = this.tx
  this.setDispenseWaitEnded(false)
  return this.trader.waitForDispense(this.tx, status, this.setDispenseWaitEnded)
    .then(tx => {
      if (!tx) return
...
Brain.prototype.gotoPendingDeposit = function gotoPendingDeposit () {
