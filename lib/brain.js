...
Brain.prototype._pollForLiveView = function _pollForLiveView () {
  const isLiveViewOn = () =>
    got.get("http://localhost:4321/ping", { timeout: 50 }) // 50ms
      .then(() => true, () => false)

  const liveViewIntervalHandle = setInterval(
    () => isLiveViewOn().then(isOn => {
      if (isOn) {
        this.browser().send({ action: 'enableLiveView' })
        clearInterval(liveViewIntervalHandle)
      }
    }), 100) // 100ms

  return () => clearInterval(liveViewIntervalHandle)
}
...
Brain.prototype._startAddressScan = function _startAddressScan () {
  ...
  const stopLiveViewPolling = this._pollForLiveView()
  const frames = []
