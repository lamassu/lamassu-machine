...
const COMPLIANCE_VERIFICATION_STATES = ['smsVerification', 'permission_id', 'permission_face_photo', 'usSsnPermission', 'customInfoRequestPermission', 'externalCompliance']
...
Brain.prototype.transitionScreen = function transitionScreen () {
  ...
  if (!appState) {
    ...
    if (!appState) {
      ...
      machineState = external.getState()
      switch (machineState) {
        case 'askForCustomInfoRequest':
          appState = 'inputCustomInfoRequest'
          break
...
Brain.prototype.triggerExternal = function triggerExternal (trigger) {
  ...
  return this.trader.getExternalCompliance(customer.id, trigger.id)
    .then(result => {
...
Brain.prototype.showExternalCompliance = function showExternalCompliance (url) {
  if (this.tx.direction === 'cashOut') {
    ...
    return this._screenTimeout(() => this.finishBeforeSms(), this.config.externalValidationTimeout)
  }
...
Brain.prototype.isTierCompliant = function isTierCompliant (trigger, triggerTx, requiredTiers, _tier) {
  const tx = this.tx
  const customer = this.customer || {}

  const tier = !_.isNil(trigger) ? trigger.requirement : _tier

  const getCustomerExternalValidationStatus = (externalData, trigger) => {
    const requiredLevel = trigger.externalServiceApplicantLevel
    if (_.isNil(externalData.review)) return 'pending'

    const isSameReviewLevel = requiredLevel === externalData.review.levelName
    const reviewStatus = isSameReviewLevel && externalData.review.reviewStatus
    const reviewResult = isSameReviewLevel && externalData.review.reviewResult.reviewAnswer

    return reviewStatus === 'completed' ? reviewResult === 'GREEN' ? 'verified' : 'blocked' : 'pending'
  }
  ...
  switch (tier) {
    ...
    case 'suspend':
      return false
    case 'custom':
      const customInfoRequestId = trigger.customInfoRequestId
    ...
    case 'external':
      ...
      const verificationStatus = getCustomerExternalValidationStatus(customer.externalCompliance, trigger)
      if (verificationStatus === 'verified') return true
      if (verificationStatus === 'blocked' || verificationStatus === 'pending') return false
    default:
...
Brain.prototype.requiredTiers = function requiredTiers (triggers, history, triggerTx) {
  this.triggers = getTriggered(triggers, history, triggerTx)
  const getHighestSuspend = _.compose(_.get('id'), _.maxBy('suspensionDays'), _.filter({ requirement: REQUIREMENTS.SUSPEND }))

  this.suspendTriggerId = getHighestSuspend(this.triggers)

  const requirements = _.uniq(_.map(_.get('requirement'))(this.triggers))
  const unorderedTiers = _.isEmpty(requirements) ? [] : _.union(requirements, ['sms'])

  const requiredTiers = _.sortBy(req => _.indexOf(req, ORDERED_REQUIREMENTS))(unorderedTiers)

  this.requirementAmountTriggered = getLowestAmountPerRequirement(this.triggers)
...
Brain.prototype.nonCompliantTiers = function nonCompliantTiers (triggers, history, triggerTx) {
  ...
  return _.filter(tier => {
    const trigger = _.find(['requirement', tier])(this.triggers)
    return !this.isTierCompliant(trigger, triggerTx, requiredTiers, tier)
  }, requiredTiers)
}
