...
var Brain = function(config) {
  ...
  var billDispenserConfig = config.billDispenser;
  if (this.twoWayMode) {
    this.billDispenser = config.mockBillDispenser ?
      require('./mocks/billdispenser').factory() :
      require('./billdispenser').factory(billDispenserConfig);
  }
...
Brain.prototype._initTraderEvents = function _initTraderEvents() {
  ...
  this.trader.on('dispenseUpdate', function(status) { self._dispenseUpdate(status); });
...
Brain.prototype._idle = function _idle() {
  ...
  if (this.twoWayMode) this._idleTwoWay();
  else this._idleOneWay();
};

Brain.prototype._idleTwoWay = function _idleTwoWay() {
  // We've got our first contact with server, load up billDispenser
  var self = this;
  ...
  this.billDispenser.init({
    ...
    virtualCartridges: this.trader.virtualCartridges,
...
Brain.prototype._fakeIdle = function _fakeIdle() {
  var idleState = this.twoWayMode ? 'fakeDualIdle' : 'fakeIdle';
...
Brain.prototype._getFiatButtonResponse = function _getFiatButtonResponse() {
  ...
  var activeDenominations =
    this.billDispenser.activeDenominations(this.trader.fiatTxLimit, tx.fiat);
  var remaining = this.trader.fiatTxLimit - tx.fiat;

  var response = {
    credit: tx,
    activeDenominations: activeDenominations,
    txLimitReached: remaining === 0
  };
...
Brain.prototype._cashOut = function _cashOut() {
  ...
  // TODO this should go in the dispenseCompleted call
  // when implemented
  var billDistribution = this.billDispenser.billDistribution(tx.fiat);
  tx.billDistribution = billDistribution;

  this._transitionState('deposit', {tx: tx});

  var self = this;
  this.trader.cashOut(tx, function (err, bitcoinAddress) {
    if (err) throw new Error(err);    // TODO handle better
...
Brain.prototype._dispenseUpdate = function _dispenseUpdate(dispenseStatus) {
  ...
  switch(status) {
    case 'timeout':
    case 'insufficientDeposit':     // TODO should be its own state
      this._idle();
      break;
    case 'authorizedDeposit':
      this.billDispenser.dispense(dispenseStatus.dispenseFiat, function (err, result) {
        if (err) throw new Error(err);    // TODO special error screen

        // TODO: report accepted and rejected to server via dispenseAck
        // TODO: if accepted don't add up to dispenseFiat, display error screen

        var sessionId = self.sessionId;
        setTimeout(function () {
          if (self.state === 'fiatComplete' && self.sessionId === sessionId)
            self._completed();
        }, 60000);
        self._transitionState('fiatComplete', {tx: self.fiatTx});
      });
...
function determineDevicePaths(config) {
  config.id003.device = determineDevicePath(config.id003.device);
