...
Brain.prototype._physicalDispense = function _physicalDispense () {
  ...
  const notes = []
  _.forEach(it => notes.push(this.tx.bills[it].provisioned), _.times(_.identity(), this.numberOfCassettes + this.numberOfStackers * 2))
  ...
  const initialParams = {
    ...
    batchAmount,
    currentBatch: 1,
    dispenseRecords: _.map(() => ({ dispensed: 0, rejected: 0 }), _.times(_.identity(), this.numberOfCassettes + this.numberOfStackers * 2))
  }
