...
Brain.prototype._start = function _start () {
  ...
  if (printPaperWallet) {
    ...
    kioskPrinter.checkStatus(deviceConfig.kioskPrinter, statusQueryTimeoutMs)
      .then((printerStatus) => {
        console.log('Kiosk printer status: ', printerStatus)
        if (printerStatus.hasErrors) {
          console.log('[ERROR]: The kiosk printer is in an invalid state.')
          this._idle()
        }
        ...
        kioskPrinter.printWallet(wallet, printerCfg)
          .then(() => {
            ...
            this._handleScan(wallet.publicAddress)
          })
          .catch((err) => {
