...
Brain.prototype._billsRead = function _billsRead (_data) {
  ...
  if (remainingFiatToInsert.lt(currentBills)) {
    ...
    if (highestBill.lte(0)) {
      ...
      return this._billsReadResolver({ success: false })
    }
  ...
  if (!isCompliant) {
    ...
    if (idTier) return this._billsReadResolver({ success: false, callback: this.transitionToVerificationScreen, args: [nonCompliantTier] })

    return this._billsReadResolver({ success: false, callback: this.runComplianceTiers, args: [nonCompliantTiers] })
  }

  // verify if any non compliant manual trigger was scanned and end tx
  if (this.txBlockedByManualTrigger) {
    this.txBlockedByManualTrigger = false
    return this._billsReadResolver({ success: false, callback: this.showBlockedCustomer, args: [] })
  }
  ...
  return this._billsReadResolver({ success: true })
}

Brain.prototype._billsReadResolver = function _billsReadResolver ({ success, callback, args }) {
  // if (success) {
...
Brain.prototype.recyclerFinish = function recyclerFinish (bills, blockedCustomer) {
