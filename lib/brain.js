...
Brain.prototype.traderRun = function traderRun (cb) {
  this.trader.poll(true, cb)
  this.pollHandle = setInterval(() => {
    this.trader.poll(this.forceLatestConfig())
  }, POLL_INTERVAL)
}
...
Brain.prototype._connectedBrowser = function _connectedBrowser () {
  ...
  var wifiList = this.state === 'wifiList' && this.wifis
  ? rec.wifiList = this.wifis
  : []
...
Brain.prototype._dbPath = function _dbPath () {
  return path.resolve(dataPath, 'db.json')
}
...
Brain.prototype.forceLatestConfig = function forceLatestConfig () {
  return this.isIdleState() || this.beforeIdleState
}
...
Brain.prototype._pollUpdate = function _pollUpdate () {
  ...
  if (this.forceLatestConfig()) this.trader.setConfigVersion()
...
Brain.prototype._networkUp = function _networkUp () {
  ...
  this.networkDown = false
  if (_.contains(['networkDown', 'connecting', 'wifiConnected', 'pairing'], this.state)) {
    this._restart()
  }
}
...
Brain.prototype._resetCartridges = function _resetCartridges () {
  ...
  var oldCartridges = this.db.dispense.cartridges

  if (!cartridges) return
  if (R.equals(cartridges, oldCartridges)) return
