...
Brain.prototype.activate = function activate () {
  ...
  if (config.mockTrader) {
    this.trader = require('./mocks/trader')(protocol, this.clientCert, connectionInfo, this.dataPath, cryptomatModel)
  } else {
    this.trader = require('./trader')(protocol, this.clientCert, connectionInfo, this.dataPath, cryptomatModel)
  }
...
Brain.prototype.getHardLimitThreshold = function getHardLimitThreshold (trader, hasFFcamera) {
  const thresholdIfNoFrontCamera = hasFFcamera ? BN(Infinity) : BN(trader.frontCameraVerificationThreshold)
  return BN.klass.min(trader.hardLimitVerificationThreshold, thresholdIfNoFrontCamera)
}
...
Brain.prototype.hardLimit = function hardLimit () {
  ...
  const hardLimitThreshold = this.getHardLimitThreshold(trader, this.hasFrontFacingCamera)
  return BN.klass.min(hardLimitThreshold, failedTierThreshold)
}
