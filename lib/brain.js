...
Brain.prototype.processAction = function processAction (action, stateMachine) {
  switch (action) {
    ...
    case 'saveUsSsn':
      ...
      break
    case 'test':
      console.log('a', 'AAAAAA', 'CUSTOM INFO REQUEST AAAAAA')
      break
  }
}
...
Brain.prototype.transitionScreen = function transitionScreen () {
    ...
    if (!appState) {
      ...
      switch (machineState) {
        case 'customPermissionScreen2':
          appState = 'customPermissionScreen2'
          break
        case 'authorizing':
          appState = 'waiting'
          break
  ...
  if (!appState) { return }
  const customInfoRequest = appState === 'customPermissionScreen2'
    ? { customInfoRequest: _.get('customInfoRequest.customRequest')(_.find(trigger => trigger.customRequirementId === this.customRequirementId)(this.triggers)) }
    : null
...
Brain.prototype._processReal = function _processReal (req) {
  ...
  switch (req.button) {
    ...
    case 'submitPromoCode':
      ...
      break
    case 'permissionCustomPermission':
      this.permissionsGiven[this.customRequirementId] = true
      this._continueSmsCompliance()
      break
    case 'cancelCustomPermission':
      this.failedCompliance = this.customRequirementId
      this.failedComplianceValue = this.requirementAmountTriggered[this.failedCompliance]
...
Brain.prototype.isTierCompliant = function isTierCompliant (tier) {
  ...
  switch (tierOrCustom) {
    ...
    case 'suspend':
      return false
    case 'custom':
      const customRequirementId = tier
      const customerData = _.find(['info_request_id', customRequirementId])(customer.customInfoRequestData)
      return customerData && customerData.approved
    default:
...
Brain.prototype.nonCompliantTiers = function nonCompliantTiers (triggers, history, triggerTx) {
  ...
  const triggeredFormatted = _.map(o => o.customRequirementId ? { ...o, requirement: o.customRequirementId } : o, triggered)
  ...
  console.log('a', triggeredFormatted)
  ...
  console.log('a', 'requiredTiers', requiredTiers)
...
Brain.prototype.runComplianceTiers = function (nonCompliantTiers) {
  ...
  const runCustomComplianceTier = customTier && _.get(tier)(this.permissionsGiven)

  console.log('a', 'b', customTier, !_.get(tier)(this.permissionsGiven), this.permissionsGiven)
...
Brain.prototype.transitionToVerificationScreen = function (tier) {
  ...
  switch (formattedTier) {
    ...
    case 'usSsn':
      ...
      break
    case 'custom':
      this._transitionState('customPermission', {
        tx: this.tx,
        customInfoRequest: _.get('customInfoRequest.customRequest')(_.find(trigger => trigger.customRequirementId === this.customRequirementId)(this.triggers))
      })
      break
