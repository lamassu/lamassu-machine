...
Brain.prototype._idle = function _idle (locale) {
  ...
  this.lastRejectedRecyclerFiat = BN(0)
...
Brain.prototype.complianceAmount = function complianceAmount () {
  ...
  const lastRejectedFiat = this.isCashRecycler(this.billValidator)
    ? this.lastRejectedRecyclerFiat
...
Brain.prototype.recyclerBillsRead = function recyclerBillsRead (data) {
  const billValidator = this.billValidator
  ...
  const bills = this.buildRecyclerBills(data)
  ...
  if (remainingFiatToInsert.lt(currentBills)) {
    billValidator.reject()
    this.lastRejectedRecyclerFiat = currentBills
...
Brain.prototype.buildRecyclerBills = function buildRecyclerBills (bills) {
...
Brain.prototype.recyclerContinue = function recyclerContinue () {
  return this.billValidator.cashCount()
    .then(bills => Promise.all([bills, this.recyclerBillsRead(bills)]))
    .then(([bills, res]) => {
      if (res.success) {
        const billsObjArr = this.buildRecyclerBills(bills)
        return this.recyclerBillFinish(billsObjArr)
      }
