...
const { getAmountToHardLimit, getTriggered } = require('./compliance/triggers/triggers')
...
Brain.prototype.run = function run () {
  ...
  this.hasFrontFacingCamera = this.scanner.hasCamera('frontFacingCamera')
...
// TODO new-admin Take overrides into consideration
...
Brain.prototype.smsFlowHandleReturnState = function smsFlowHandleReturnState () {
  ...
  const now = new Date()
  const isSuspended = this.customer.suspendedUntil && new Date(this.customer.suspendedUntil) > now
  if (isStartOfTx && isSuspended) {
    var diff = new Date(this.customer.suspendedUntil).valueOf() - now.valueOf()
    // var diffInDays = _.floor(diff / 1000 / 60 / 60 / 24)

    // TODO new-admin we should pass along days/hours/weeks to the ui
    // - (diffInDays * 24))
    var diffInHours = _.ceil((diff / 1000 / 60 / 60))

    const data = { hardLimitHours: diffInHours }
...
Brain.prototype.nonCompliantTiers = function nonCompliantTiers (triggers, history, triggerTx) {
  ...
  const requiredTiers = _.sortBy(name => _.indexOf(ORDERED_REQUIREMENTS, name.name))(unorderedTiers)
...
Brain.prototype._billRead = function _billRead (data) {
  ...
  const remainingFiatToInsert = this.balance().sub(fiatBeforeBill)
