...
const Brain = function (config) {
  ...
  if (config.billValidatorDevice) config.billValidator.rs232.device = config.billValidatorDevice

  config.billValidator.fiatCode = this.fiatCode
...
Brain.prototype.selectBillValidatorClass = function selectBillValidatorClass () {
  ...
  if (this.config.billValdiator.deviceType === 'cashflowSc') return require('./mei/cashflow_sc')
...
Brain.prototype.loadBillValidator = function loadBillValidator () {
  ...
  return billValidatorClass.factory(this.config.billValidator)
}
...
Brain.prototype.authorizeIdCardData = function () {
  ...
  this.trader.updateCustomer(customer.id, {idCardData: data})
    .then(result => {
...
Brain.prototype._processReal = function _processReal (req) {
  switch (req.button) {
    ...
    case 'blockedCustomerOk':
      ...
      break
    case 'lightningYes':
      return this.lightningMode(true)
    case 'lightningNo':
      return this.lightningMode(false)
  }
}
...
Brain.prototype.isBlocked = function isBlocked (customer) {
  ...
  return _.some(tier => this.isTierBlocked(tier), tierNames)
}
...
Brain.prototype.smsFlowHandleReturnState = function smsFlowHandleReturnState () {
  // No need to check compliance on redeem, since tx was already checked.
  if (this.redeem) {
...
function chooseBillDispenser (config) {
  const billDispenser = config.model

  console.log(config)
  console.log(billDispenser)

  return billDispenser === 'f56'
    ? require('./f56/f56-dispenser').factory(config)
    : require('./puloon/puloon-dispenser').factory(config)
}
...
Brain.prototype._idleTwoWay = function _idleTwoWay (localeInfo) {
  ...
  if (!this.billDispenser) {
    this.billDispenser = this.rootConfig.mockBillDispenser
      ...
      : chooseBillDispenser(this.rootConfig.billDispenser)
  }
...
Brain.prototype.commitCashOutTx = function commitCashOutTx (isLightning) {
  return this.sendCashOutTx(isLightning)
    .then(ntx => {
      ...
      const toAddress = coinUtils.formatAddress(ntx.cryptoCode, ntx.toAddress)
      const depositInfo = _.set('toAddress', toAddress, ntx)
      return this.browser().send({depositInfo, depositUrl})
    })
    .catch(err => this._fiatError(err))
}

Brain.prototype.sendCashOutTx = function sendCashOutTx (isLightning) {
  return this.updateTx({isLightning})
}
...
Brain.prototype.toDeposit = function toDeposit () {
  const tx = this.tx
  const hasLightning = tx.cryptoCode === 'BTC' && this.trader.hasLightning

  if (hasLightning) return this._timedState('chooseLightning')
  return this.lightningMode(hasLightning)
}

Brain.prototype.lightningMode = function lightningMode (isLightning) {
  const tx = this.tx
  this._transitionState('deposit', {tx})
  return this.commitCashOutTx(isLightning)
}
