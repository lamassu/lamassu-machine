...
Brain.prototype.authorizeIdCardData = function authorizeIdCardData () {
  return Promise.resolve()
    .then(() => {
      ...
      return this.trader.updateCustomer(customer.id, {idCardData: data}, this.tx.id)
    })
    .then(result => {
...
Brain.prototype._connectedBrowser = function _connectedBrowser () {
  ...
  if (!this.trader || !this.trader.coins) {
    const rec = {
      ...
      locale: 'en-US',
      cryptomatModel,
      operatorInfo: this.trader ? this.trader.operatorInfo : { active: false }
    }
...
Brain.prototype.smsFlowHandleReturnState = function smsFlowHandleReturnState () {
  ...
  const cashInAmount = this.billValidator.lowestBill(coin.minimumTx).add(amount)
  const cashOutAmount = this.minimumFiat().add(amount)

  const cashInHardLimit = this.tx.direction === 'cashIn' && BN(0).eq(this.tx.fiat) && complianceTiers.dailyLimitReached(this.trader, cashInAmount)
  const cashOutHardLimit = this.tx.direction === 'cashOut' && complianceTiers.dailyLimitReached(this.trader, cashOutAmount)

  if (cashInHardLimit || cashOutHardLimit) {
...
Brain.prototype._privateWalletPrinting = function _privateWalletPrinting () {
  ...
  const statusQueryTimeoutMs = 2000
  return kioskPrinter.checkStatus(deviceConfig.kioskPrinter, statusQueryTimeoutMs)
    .then((printerStatus) => {
...
Brain.prototype._billRead = function _billRead (data) {
  ...
  if (!this.isCompliant(amount)) {
    ...
    if (this.tx.phone) {
      return this._transitionState('idVerification')
    }
...
Brain.prototype._billRejected = function _billRejected () {
  const self = this
  if (!_.includes(this.state, BILL_ACCEPTING_STATES) && this.state !== 'smsVerification' && this.state !== 'idVerification') return
...
Brain.prototype._doSendCoins = function _doSendCoins () {
  if (this.state !== 'acceptingBills' && this.state !== 'smsVerification' && this.state !== 'idVerification') return
...
Brain.prototype._sendCoinsError = function _sendCoinsError (err) {
  ...
  if (this.tx.isPaperWallet) {
    this._printCashInReceipt()
  }
}
...
Brain.prototype._printCashInReceipt = function _printCashInReceipt () {
