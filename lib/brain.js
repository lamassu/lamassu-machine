...
Brain.prototype._initTraderEvents = function _initTraderEvents () {
  ...
  this.trader.on('disable', function () { self._disable() })
  this.trader.on('enable', function () { self._enable() })
...
Brain.prototype.isStaticState = function isStaticState () {
  const staticStates = ['chooseCoin', 'idle', 'pendingIdle', 'dualIdle',
    'networkDown', 'unpaired', 'maintenance', 'virgin', 'wifiList', 'disabled']
...
const isNonTx = state => _.includes(state, NON_TX_STATES) && state !== DISABLED_STATE
...
Brain.prototype._disable = function _disable () {
  if (this.state !== 'disabled') {
    console.log('Remote disable')
    this._setState('disabled')
    this.browser().send({ action: 'disabled' })
  }
}

Brain.prototype._enable = function _enable () {
  if (this.state === 'disabled') {
