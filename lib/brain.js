...
Brain.prototype._pair = function _pair (totem) {
  ...
  const cassettes = _.isNumber(deviceConfig.billDispenser.cassettes)
    ? deviceConfig.billDispenser.cassettes
    : 2

  const model = platformDisplay(this.determinePlatform())
  return pairing.pair(totem, this.clientCert, this.connectionInfoPath, model, cassettes)
    .then(() => this.activate())
    .catch(err => {
      console.log(err.stack)
...
Brain.prototype._idleTwoWay = function _idleTwoWay (localeInfo) {
  ...
  console.log('uicassettes', uiCassettes)
...
Brain.prototype._batchDispense = function _batchDispense (notesToDispense, { batchAmount, dispenseRecords, currentBatch, error }) {
  ...
  console.log('dispenseRecords', dispenseRecords)
...
Brain.prototype._physicalDispense = function _physicalDispense () {
  ...
  const numOfCassettes = _.isNumber(deviceConfig.billDispenser.cassettes)
    ? deviceConfig.billDispenser.cassettes
    : 2

  const notes = []
  _.forEach(it => notes.push(this.tx.bills[it].provisioned), _.times(_.identity(), numOfCassettes))
  ...
  const initialParams = {
    ...
    batchAmount,
    currentBatch: 1,
    dispenseRecords: _.map(() => ({ dispensed: 0, rejected: 0 }), _.times(_.identity(), numOfCassettes))
  }
