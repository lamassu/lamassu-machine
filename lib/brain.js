...
Brain.prototype._loadDb = function _loadDb () {
  var dispenseTemplate = {reserved: {sessions: {}}}
...
function dispenserBalance (cartridges) {
  function cartridgeBalance (cartridge) {
    return cartridge.count * cartridge.denomination
  }
  return R.sum(R.map(cartridgeBalance, cartridges))
}

function computeDenominationActiveMap (cartridges, virtualCartridges, credit) {
  var balance = dispenserBalance(cartridges)
  var effectiveBalance = balance - credit
  var denoms = R.union(virtualCartridges, R.pluck('denomination', cartridges))
  return R.fromPairs(R.zip(denoms, R.map(R.gte(effectiveBalance), denoms)))
}
...
Brain.prototype._getFiatButtonResponse = function _getFiatButtonResponse () {
  ...
  var balance = dispenserBalance(cartridges)
  var reserved = totalReserved(this.db.dispense.reserved.sessions)
  var credit = tx.fiat + reserved
  var isEmpty = balance - credit <= 0
  var txLimitReached = tx.fiat >= txLimit
  var activeMap = computeDenominationActiveMap(cartridges, virtualCartridges, credit)
  var response = {
    credit: tx,
    activeDenominations: {
      isEmpty: isEmpty,
...
function totalReserved (sessions) {
  return R.sum(R.pluck('amount', R.values(sessions)))
}
...
Brain.prototype._redeemDispense = function _redeemDispense () {
  ...
  var cartridges = this.trader.cartridges
  var db = this.db
  var reserved = totalReserved(db.dispense.reserved.sessions)
  var isReserved = !!db.dispense.reserved.sessions[tx.sessionId]
  var balance = dispenserBalance(cartridges)
  ...
  if (balance < amount + reserved && !isReserved) {
    return this._timedState('outOfCash')
  }

  this._unreserveBills(tx.sessionId)
  this._dispense()
}
...
Brain.prototype._redeemLater = function _redeemLater () {
  var tx = this.fiatTx
  this._reserveBills(tx.sessionId, tx.fiat, tx.currencyCode)
...
Brain.prototype._expireReservedBills = function _expireReservedBills () {
  var self = this
  var targetTimestamp = Date.now - 24 * 60 * 60 * 1000
  var isExpired = R.gt(targetTimestamp)
  ...
  function expireBills () {
    ...
    var patch = R.map(deleteRec, expired(self.db.dispense.reserved))
    self._saveDb(patch)
  }

  setInterval(expireBills, 60 * 60 * 1000)
}

Brain.prototype._reserveBills = function _reserveBills (sessionId, amount, currency) {
