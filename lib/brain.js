...
Brain.prototype._chooseCoinScreen = function _chooseCoinsScreen (localeInfo, cassettes) {
  if (this.singleCrypto()) {
    const cryptoCode = this.trader.coins[0]
    this.browser().send({
      localeInfo: localeInfo,
      cassettes: cassettes,
      coins: this.trader.coins,
      twoWayMode: this.twoWayMode(),
      cashInFee: this.trader.coinSettings[cryptoCode].cashInFee
    })
...
Brain.prototype._start = function _start () {
  ...
  const coinSettings = this.trader.coinSettings[this.tx.cryptoCode]
  const update = _.assignAll([this.tx, {direction: 'cashIn'}, coinSettings])
...
Brain.prototype._waitForDispense = function _waitForDispense (status) {
  ...
  .catch(() => {
    this.fastUpdateTx({timedout: true})
