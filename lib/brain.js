...
Brain.prototype._processReal = function _processReal (req) {
  ...
  switch (req.button) {
    ...
    case 'cancelCustomInfoRequest':
      console.log('a', 'cancel custom info request')
...
Brain.prototype.isTierCompliant = function isTierCompliant (tier) {
  ...
  switch (tierOrCustom) {
    ...
    case 'custom':
      ...
      if (!customerData || customerData.approved === false) return false
      // when approved is null then data was already added but not manually approved yet by the operator
      return true
    default:
...
Brain.prototype.nonCompliantTiers = function nonCompliantTiers (triggers, history, triggerTx) {
  ...
  console.log('a', 'nonCompliantTiers', triggered)
...
Brain.prototype._chooseFiat = function _chooseFiat () {
  const amount = this.complianceAmount()
  console.log('a', 'chooseFiat', amount.toString())
  const triggerTx = { fiat: amount, direction: 'cashOut' }
  const nonCompliantTiers = this.nonCompliantTiers(this.trader.triggers, this.customerTxHistory, triggerTx)
  console.log('a', 'chooseFiat', nonCompliantTiers)
  console.log('a', '----------', 'chooseFiat', this.trader.triggers, this.customerTxHistory, triggerTx)
  const isCompliant = _.isEmpty(nonCompliantTiers)
