...
const factoryResetHelper = require('./factory-reset-helper')
...
Brain.prototype._connectedBrowser = function _connectedBrowser () {
  ...
  const fullRec = {
    ...
    manufacturingMode: this.trader.manufacturingMode,
    wifiList: wifiList,
    rates
  }
...
Brain.prototype._factoryReset = function _factoryReset () {
  factoryResetHelper.nuke(this.dataPath)
    .then(() => this.trader.unpair()
      .catch(err => console.log('failure removing pairing on the server during factory reset', err))
    )
    .then(() => this._transitionState('virgin'))
    .catch(err => this._factoryResetError(err))
}

Brain.prototype._factoryResetError = function _factoryResetError (err) {
  console.log('failure factory resetting machine', err)
  this._timedState('factoryResetError')
}
