...
Brain.prototype._syncDeviceConfig = function _syncDeviceConfig () {
  const deviceConfigPath = path.resolve(__dirname, '../device_config.json')
  const writeFile = pify(fs.writeFile)

  this.trader.syncDeviceConfig(deviceConfig)
    .then(({ body: { config: newDeviceConfig, triggerRestart } }) => Promise.all([writeFile(deviceConfigPath, JSON.stringify(newDeviceConfig, null, 2), 'utf-8'), triggerRestart]))
    .then(([_, triggerRestart]) => {
      if (triggerRestart)
        return this._restartServices('Device config sync', true)
    })
}
...
Brain.prototype._initTraderEvents = function _initTraderEvents () {
  ...
  this.trader.on('syncDeviceConfig', function () { self._syncDeviceConfig() })
}
