...
Brain.prototype._start = function _start () {
  ...
  const updateRec = {
    ...
    minimumTx: coin.minimumTx
  }
  const update = _.assignAll([this.tx, updateRec, _.omit('display', coin)])
...
Brain.prototype.updateTx = function updateTx (updateTx) {
  console.log('DEBUG899: %s', this.tx.txVersion)
  const newTx = Tx.update(this.tx, updateTx)
  this.tx = newTx
  console.log('DEBUG890: %s', this.tx.txVersion)
