...
var STATIC_STATES = [State.IDLE, 'pendingIdle', 'dualIdle', 'networkDown', 'unpaired'];
...
Brain.prototype.run = function run() {
  ...
  var callback = function() { self._transitionState('restart'); console.log('Scheduled restart after idle time.'); process.exit(); };
  
  this._executeCallbackAfterASufficientIdlePeriod(callback);
};

Brain.prototype._executeCallbackAfterASufficientIdlePeriod = function _executeCallbackAfterASufficientIdlePeriod(callback) {
	var self = this;
