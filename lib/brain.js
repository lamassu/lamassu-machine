...
Brain.prototype._idle = function _idle() {
  ...
  this.trader.sessionId = this.sessionId;
...
Brain.prototype._chooseFiat = function _chooseFiat() {
  ...
  var interval = setInterval(function () {
    var doClear = self.state !== 'chooseFiat' || self.sessionId !== sessionId;
...
function fillInBills(tx, bills) {
  ...
  for (var i = 0; i < len; i++) {
    tx.billDistribution[i].actualDispense = bills[i].accepted;
    tx.billDistribution[i].rejected = bills[i].rejected;
  }
}
