...
var Brain = function(config) {
  ...
  if (config.mockScanId) {
    this.scanId = require('./mocks/scanid').factory(config.scanId);
  } else {
    this.scanId = require('./compliance/scanid').factory(config.scanId);
  }
  ...
  if (this.twoWayMode) {
    this.billDispenser = config.mockBillDispenser ?
      ...
      require('./billdispenser').factory({device: '/dev/ttyS0'});
  }
...
Brain.prototype._idle = function _idle() {
  ...
  if (this.twoWayMode)
    this._transitionState('dualIdle', {localeInfo: this.localeInfo});
  else
    this._transitionState('idle', {localeInfo: this.localeInfo});
};

Brain.prototype._start = function _start() {
  var idVerificationLimit = this.trader.idVerificationLimit;
  var checkId = idVerificationLimit !== null &&
    idVerificationLimit <= this.billValidator.lowestBill();
...
Brain.prototype._startIdScan = function _startIdScan() {
  this._transitionState('scanId');
  this.idVerify.reset();
  var self = this;
  this.billValidator.lightOn();
  this.scanId.scan(function (err, result) {
...
Brain.prototype._verifyId = function _verifyId(options) {
  ...
  var self = this;
  this.idVerify.verify(function (err, result) {
    if (!err && result.success)
      return self._startAddressScan();
...
Brain.prototype._dispenseUpdate = function _dispenseUpdate(dispenseStatus) {
  ...
  console.log('DEBUG in dispenseUpdate');
  ...
  switch(status) {
    ...
    case 'insufficientDeposit':     // TODO should be its own state
      ...
      break;
    case 'fullDeposit':
      this.billDispenser.dispense(dispenseStatus.dispenseFiat, function (err) {
