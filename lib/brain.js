...
Brain.prototype.waitForCollection = function waitForCollection (dispenseGenerator) {
  const txId = this.tx.id
  let g = dispenseGenerator.call(this) // change execution context with .call
  let start = true
  let yieldedVal

  const checkCollection = () => {
    const isCollecting = this.tx.id === txId && this.state === 'dispensing'

    if (start) yieldedVal = g.next()
    start = false
...
Brain.prototype._physicalDispense = function _physicalDispense () {
  ...
  function optimize(inputArr, limit) {
    if (limit === undefined) return [inputArr]

    let inputHead = inputArr.slice(-2)[0]
    let inputTail = inputArr.slice(-1)[0]
    let optimizedArray = []
  
    function findLimitSum(a, b) {
      let head = a
      let tail = b
      ...
      optimizedArray.push([head, tail])
      if ( a === 0 && b === 0) {
          optimizedArray.pop()
          return
        }
      findLimitSum(a - head, b - tail, limit)
    }
  
    findLimitSum(inputHead, inputTail)

    return optimizedArray
  }

  notesToDispense = optimize(notes, this.billDispenser.dispenseLimit)
  ...
  self.browser().send({dispenseBatch: {current: batchNum, of: notesToDispense.length} })
  this._transitionState('dispensing')
  emit('billDispenserDispensing')
  
  function* dispenseGenerator() {
    let result = []
    for (let index = 0; index < notesToDispense.length; index++) { // cannot yield inside forEach
      let yielded = yield this.billDispenser.dispense(notesToDispense[index])
  ...
  this.waitForCollection(dispenseGenerator)
}
