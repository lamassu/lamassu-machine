...
const certPath = path.resolve(dataPath, 'cert.json')
...
const Brain = function (config) {
  ...
  path.resolve(dataPath, wifiConfig.wpaConfigPath)
  if (config.mockWifi) {
...
Brain.prototype._connect = function _connect () {
  this.clientCert = pairing.getCert(certPath)
...
Brain.prototype.activate = function activate () {
  const connectionInfo = pairing.connectionInfo(connectionInfoPath)
...
Brain.prototype._pair = function _pair (totem) {
  ...
  return pairing.pair(totem, this.clientCert, connectionInfoPath)
  .then(() => this.activate())
  .catch(err => {
    console.log(err.stack)
...
Brain.prototype._idleTwoWay = function _idleTwoWay (localeInfo) {
  const self = this
  const cartridges = this.cartridges()
...
Brain.prototype.balance = function balance () {
  const cryptoCode = this.cryptoCode
...
Brain.prototype._billRead = function _billRead (data) {
  ...
  const totalFiat = this.tx.fiat.add(this.pending.fiat)
...
function txMerger (objValue, srcValue, key, object, source, stack) {
  if (key === 'bills') return _.unionBy(_.get('id'), srcValue, objValue)
  if (key === 'fiat' || key === 'cryptoAtoms') return srcValue.add(objValue)
}

Brain.prototype.updateTx = function updateTx (newTx) {
  this.tx = _.mergeWith(txMerger, newTx, this.tx)
...
Brain.prototype._billValid = function _billValid () {
  ...
  const totalFiat = this.credit.fiat + this.pending.fiat
  const fiatBalance = this.balance() - totalFiat
  ...
  this.pending = null
  const cryptoAtoms = this.truncateCrypto(this.credit.cryptoAtoms.plus(pending.cryptoAtoms))
...
Brain.prototype._billRejected = function _billRejected () {
  ...
  this.pending = null
  const returnState = this.credit.fiat === 0
  ? 'acceptingFirstBill'
  : 'acceptingBills'
  ...
  const credit = credit.fiat && credit.fiat !== 0
  ? this._uiCredit()
  : null
...
Brain.prototype._uiCredit = function _uiCredit () {
  ...
  let fiat = credit.fiat
  let cryptoAtoms = credit.cryptoAtoms
  let lastBill = null
...
Brain.prototype.truncateCrypto = function truncateCrypto (cryptoAtoms) {
  const scale = coins[this.cryptoCode].displayScale
...
Brain.prototype._createPendingTransaction = function _createPendingTransaction (bill) {
  ...
  const unitScale = coins[this.cryptoCode].unitScale
  const unitScaleFactor = new BigNumber(10).pow(unitScale)
  const exchangeRate = this.trader.rates(this.cryptoCode).cashIn
  const cryptoAtoms = this.truncateCrypto(new BigNumber(bill).div(exchangeRate).mul(unitScaleFactor))
...
Brain.prototype._executesendCoins = function _executesendCoins () {
  ...
  const self = this
  const cryptoAtoms = this.credit.cryptoAtoms
  ...
  this.trader.sendCoins(tx, function (err, res) {
    if (err) return self._sendCoinsError(err)

    const txId = res.txId

    if (tx.id !== txId) return
    self._cashInComplete(txId)
  })
}
...
Brain.prototype._unpair = function _unpair () {
  ...
  this.stop()
  pairing.unpair(connectionInfoPath)
...
Brain.prototype._getFiatButtonResponse = function _getFiatButtonResponse () {
  ...
  const txAmount = tx.fiat
  const effectiveBills = this._effectiveBills()

  function denominationIsAvailable (denom) {
    const pendingAmount = txAmount + denom
  ...
  const denominationIsActive = R.both(denominationUnderLimit, denominationIsAvailable)
  const denoms = R.union(virtualCartridges, R.pluck('denomination', cartridges))
  const activeDenoms = R.zip(denoms, R.map(denominationIsActive, denoms))
  const activeMap = R.fromPairs(activeDenoms)
  const noMore = !R.any(R.identity, R.values(activeMap))
  const txLimitReached = noMore && R.any(denominationIsAvailable, denoms)
...
Brain.prototype._fiatButton = function _fiatButton (data) {
  ...
  const rate = this.trader.rates(this.cryptoCode).cashOut
  const unitScale = new BigNumber(10).pow(coins[this.cryptoCode].unitScale)

  if (buttons.activeDenominations.activeMap[denomination]) {
    const cryptoAtoms = new BigNumber(denomination).div(rate).times(unitScale).truncated()
...
Brain.prototype.exceedsZeroConf = function exceedsZeroConf (tx) {
  const coin = coins[tx.cryptoCode]
...
Brain.prototype._cashOut = function _cashOut () {
  ...
  this.trader.cashOut(tx, function (err, res) {
    if (err) return self._fiatError(err)

    tx.toAddress = res.toAddress
    self.tx.started = true

    const amountStr = self.toCryptoUnits(tx.cryptoAtoms, tx.cryptoCode).toString()
    const depositUrl = coinUtil.depositUrl(tx.toAddress, tx.cryptoCode, amountStr)
    self.browser().send({depositInfo: tx, depositUrl})
...
Brain.prototype._waitForDispense = function _waitForDispense (status) {
  const self = this
  this.trader.waitForDispense(this.tx, status, function (err, res) {
    if (err) return self._timedState('depositTimeout')
    if (!res) return
...
Brain.prototype._updateCartridges = function _updateCartridges (bills) {
  const cartridges = R.clone(this.cartridges())
  const len = bills.length
  for (let i = 0; i < len; i++) {
...
Brain.prototype._physicalDispense = function _physicalDispense (tx) {
  ...
  this.billDispenser.dispense(notes, function (err, result) {
    ...
    const tx = self.tx
    let cartridges = self.cartridges()
