...
Brain.prototype._physicalDispense = function _physicalDispense () {
  ...
  const batchDispense = ({ dispensed, current, error }) => {
    // short circuit out if a error is found
    if (error) {
      return { dispensed, error }
    }

    const notes = notesToDispense[current - 1]

    this.browser().send({ action: 'dispensing', dispenseBatch: { current, of: batchAmount } })
    const addDispensed = _.zipWith((a, b) => ({ dispensed: _.sumBy('dispensed', [a, b]), rejected: _.sumBy('rejected', [a, b]) }))

    return this.billDispenser.dispense(notes, current, batchAmount)
      .then(({ value, error }) => {
        const response = { dispensed: addDispensed(value.bills, dispensed), error, current: current + 1 }
        ...
        const screen = current === batchAmount ? 'dispensingCollect' : 'dispensingPartialCollect'
        this.browser().send({ action: screen, dispenseBatch: { current: current, of: batchAmount } })

        return this.billDispenser.waitForBillsRemoved()
          .then(() => response)
          .catch(error => _.assign(response, { error }))
      })
      .catch(error => ({ dispensed, error }))
  }

  const batchesFinished = ({ dispensed: bills, current, error }) => {
    const tx = this.tx
    ...
    const fastUpdateTxEventData = _.extend({ bills: _.toArray(_.merge(tx.bills, bills)), dispenseConfirmed }, !dispenseConfirmed ? fastUpdateTxEventDataErr : {})
  ...
  const tasks = [...Array(batchAmount).fill(batchDispense), batchesFinished]
