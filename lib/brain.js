...
var Brain = function (config) {
  ...
  this.credit = {fiat: 0, cryptoUnits: new BigNumber(0), lastBill: null}
  this.creditConfirmed = {fiat: 0, cryptoUnits: new BigNumber(0)}
  this.fiatTx = null
  this.pending = null
...
Brain.prototype._chooseCoin = function _chooseCoin (coin) {
  var coins = {
    ETH: {
      unitCode: 'ETH',
      displayCode: 'mETH',
...
Brain.prototype.sendRates = function sendRates () {
  ...
  var rec = {
    currency: this.currency,
    rates: {
      rates: this.trader.rates[coin.unitCode],
      coin: coin
    }
  }
...
Brain.prototype._billValid = function _billValid () {
  ...
  this.credit.cryptoUnits = this.credit.cryptoUnits.plus(pending.cryptoUnits)
  ...
  this.trader.trade(tradeRec, function (err) {
    if (!err) {
      ...
      self.creditConfirmed.cryptoUnits = self.creditConfirmed.cryptoUnits.plus(pending.cryptoUnits)
    }
  })
  ...
  var rec = {
    ...
    cryptoUnits: pending.cryptoUnits
  }
...
Brain.prototype._uiCredit = function _uiCredit () {
  ...
  var cryptoUnits = credit.cryptoUnits
  var lastBill = null

  if (this.pending) {
    var pending = this.pending
    fiat += pending.fiat
    cryptoUnits = this.truncateCrypto(cryptoUnits.plus(pending.cryptoUnits))
...
Brain.prototype.truncateCrypto = function truncateCrypto (amount) {
  var scale = this.coin.displayScale
  var scaleFactor = Math.pow(10, scale)

  return new BigNumber(amount).truncated().div(scaleFactor)
...
Brain.prototype._createPendingTransaction = function _createPendingTransaction (bill) {
  ...
  var coinCode = this.coin.unitCode
  var exchangeRate = this.trader.rates[coinCode].cashIn
  var cryptoUnits = this.truncateCrypto(new BigNumber(bill).div(exchangeRate))
...
Brain.prototype._executesendCoins = function _executesendCoins () {
  ...
  var self = this
  var cryptoUnits = this.credit.cryptoUnits
...
Brain.prototype._cashInComplete = function _cashInComplete () {
  ...
  var rec = {
    cryptoUnits: this.credit.cryptoUnits,
    fiat: this.credit.fiat,
    currency: this.currency
  }
...
Brain.prototype._resetState = function _resetState () {
  ...
  this.cryptoAddress = null
  this.credit.fiat = 0
  this.credit.cryptoUnits = new BigNumber(0)
  this.credit.lastBill = null
  this.creditConfirmed.fiat = 0
  this.creditConfirmed.cryptoUnits = new BigNumber(0)
  this.pending = null
}
...
Brain.prototype._fiatButton = function _fiatButton (data) {
  ...
  var coinCode = this.coin.unitCode
  var rate = this.trader.rates[coinCode].cashOut

  if (buttons.activeDenominations.activeMap[denomination]) {
    var cryptoUnits = new BigNumber(denomination).div(rate).truncated()
    tx.fiat += denomination
    tx.cryptoUnits = this.truncateCrypto(tx.cryptoUnits.plus(cryptoUnits))
  }
...
Brain.prototype._clearFiat = function _clearFiat () {
  ...
  tx.fiat = 0
  tx.cryptoUnits = new BigNumber(0)
