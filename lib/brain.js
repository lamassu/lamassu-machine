...
Brain.prototype.smsCompliance = function smsCompliance (returnState) {
  ...
  flow.on('sendCode', phone => {
    ...
    .then(result => {
      console.log('sendCode response', result)
...
Brain.prototype._billRead = function _billRead (data) {
  ...
  const fiatAfterBill = this.tx.fiat.add(currentBill)
  ...
  const usersTotalDailyVolume = usersDailyVolumeBeforeTx.add(this.tx.fiat)
  ...
  const remainingFiatBeforeBill = BN(this.trader.hardLimitVerificationThreshold).sub(usersTotalDailyVolume)
  // Use the minimum available fiat
  const minRemainingFiat = BigNumber.min(availableCryptoAsFiat, remainingFiatBeforeBill)
  // Minimum allowed transaction
  const minimumAllowedTx = this.tx.minimumTx

  if (minRemainingFiat.lt(currentBill)) {
    billValidator.reject()

    const reason = remainingFiatBeforeBill.lte(availableCryptoAsFiat)
    ? 'transactionLimit'
    : 'lowBalance'

    const highestBill = billValidator.highestBill(minRemainingFiat)
