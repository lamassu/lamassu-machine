...
function txMerger (oldValue, newValue, key) {
  if (key === 'fiat') return oldValue.add(newValue)
  if (key === 'cryptoAtoms') return truncateCrypto(oldValue.add(newValue))
}

Brain.prototype.updateTx = function updateTx (updateTx) {
  const oldTx = this.tx
  const newTx = oldTx.mergeWith(txMerger, updateTx)
  this.tx = newTx

  if (oldTx.equals(newTx)) return Promise.resove(newTx)
...
Brain.prototype._billValid = function _billValid () {
  ...
  this.updateTx({
    fiat: pending.fiat,
    cryptoAtoms: pending.cryptoAtoms,
    bills: Immutable.OrderedSet.of(billRec)
  })
