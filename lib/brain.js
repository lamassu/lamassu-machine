...
Brain.prototype.isManualTier = function isManualTier (tier) {
  // BACKWARDS_COMPATIBLITY 8.1
  // older server sends triggersAutomation values as booleans (true = AUTOMATIC, false = MANUAL)
  const serverVersion = this.trader.serverVersion
  if (!serverVersion || semver.lt(serverVersion, '8.1.0-beta.0')) {
    return !_.get(tier, this.trader.triggersAutomation)
  }
...
Brain.prototype.validateTier = function validateTier (tier, triggerTx, customerTier, requiredTiers) {
  const requiredManualTiers = _.filter(tier => this.isManualTier(tier), requiredTiers)
