...
Brain.prototype.smsFlowHandleReturnState = function smsFlowHandleReturnState () {
  ...
  if (_.includes(returnState, BILL_ACCEPTING_STATES) && isCompliant) {
    ...
    this._transitionState(returnState, { tx: this.tx, smsReceiptStatus: this.trader.smsReceiptActive ? 'available' : 'disabled' })
    return this._screenTimeout(cb, this.config.billTimeout)
  }

  this._transitionState(returnState, { smsReceiptStatus: this.trader.smsReceiptActive ? 'available' : 'disabled' })
}
