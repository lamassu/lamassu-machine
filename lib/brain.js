...
Brain.prototype._startIdScan = function _startIdScan() {
  ...
  this.scanId.scan(function (err, result) {
    ...
    self._transitionState('idCode', {beep: true});
  });
};
...
Brain.prototype._idCode = function _idCode(code) {
  ...
  this.idVerify.verify(function (err, result) {
    ...
    if (err)
      return self._transitionState('idVerificationError');
    if (result.errorCode === 'codeMismatch')
      return self._transitionState('idCodeFailed');
    self._transitionState('idVerificationFailed');
  });
};
