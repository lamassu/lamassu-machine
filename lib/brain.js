...
Brain.prototype.processDispenseGeneratorAction = function processDispenseGeneratorAction (event) {
  switch (event.action) {
    ...
    case 'billDispenserPartialDispensed':
      const screen = event.current === event.of ? 'dispensingPartialCollect' : 'dispensingCollect'
      this.browser().send({ action: screen, dispenseBatch: { current: event.current, of: event.of } })
      const self = this

      let waitForF53Timeout = true
      setTimeout(() => { waitForF53Timeout = false }, 20000)
      ...
      const interval = setInterval((evt) => {
        self.billDispenser.billsPresent()
          ...
          .catch((err) => {
            if (err.code === 'DLEVEL_FSM_ERROR' || waitForF53Timeout) return
            clearInterval(interval)
