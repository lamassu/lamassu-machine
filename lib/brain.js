...
Brain.prototype._idle = function _idle (locale) {
  ...
  this.connectionInfo = pairing.connectionInfo(connectionInfoPath)
  console.log('DEBUG5: %j', this.connectionInfo)

  if (!this.connectionInfo && !this.trader.isMock) return this._unpaired()

  const config = this.rootConfig
  const protocol = config.http ? 'http:' : 'https:'

  if (config.mockTrader) {
    this.trader = require('./mocks/trader')(protocol, this.clientCert, this.connectionInfo)
  } else {
    this.trader = require('./trader')(protocol, this.clientCert, this.connectionInfo)
  }
