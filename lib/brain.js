...
var Brain = function(config) {
  ...
  this.lamassu = config.mockApi ?
    require('./mocks/lamassu') :
    require('./lamassu');
  this.lamassu.configure(config.lamassuApi);
...
Brain.prototype._idle = function _idle() {
  ...
  this.lamassu.reset(this.trader.sessionId);
...
Brain.prototype._sendSecurityCode = function _sendSecurityCode(number) {
  ...
  this.lamassu.phoneCode(number, function(err, result) {
    if (err && err.output.statusCode === 410) {
      return self._timedState('badPhoneNumber');
    }
    if (err) return self._fiatAbortedTx();
...
Brain.prototype._processPhoneNumber = function _processPhoneNumber(number) {
  ...
  if (this.redeem) {
    this.lamassu.fetchPhoneTx(number, function(err, res) {
...
Brain.prototype._redeemDispense = function _redeemDispense() {
  var self = this;
  var phoneNumber = this.currentPhoneNumber;

  // sessionId of the original deposit
  var sessionId = this.fiatTx.sessionId;

  this.lamassu.dispense(phoneNumber, sessionId, function(err) {
    if (err) return self._idle(); // TODO: already dispensed screen
    self._dispense(self.fiatTx.fiat, self.fiatTx.currencyCode);
  });
};
...
Brain.prototype._cashOut = function _cashOut() {
  ...
  this._timedState('deposit', {
    data: {tx: tx},
    timeout: 120000,
    revertState: 'fiatTimeout'
  });

  this.trader.cashOut(tx, function (err, bitcoinAddress) {
    if (err) self._idle();
    tx.toAddress = bitcoinAddress;
    self.lamassu.registerTx(tx, self.currentPhoneNumber, function(err, res) {
      // TODO: general error screen on err
      if (err) return self._idle(); // TEMP
      self._dispenseUpdate(res);
    });
...
Brain.prototype._dispense = function _dispense(fiat, currency) {
  var self = this;
...
Brain.prototype._dispenseUpdate = function _dispenseUpdate(dispenseStatus) {
  ...
  var overZeroConfLimit = this.fiatTx.fiat > this.trader.zeroConfLimit;
  switch(status) {
    ...
    case 'confirmed':
      ...
      this._dispense(dispenseStatus.fiat, this.fiatTx.currencyCode);
      break;
  }
};
