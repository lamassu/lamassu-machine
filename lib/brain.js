...
Brain.prototype._submitPromoCode = function _submitPromoCode (data) {
  ...
  this.trader.verifyPromoCode(promoCode, this.tx).then(res => {
    ...
    if (res.promoCode.discount > currentDiscount) {
      this.tx = Tx.update(this.tx, { discount: res.promoCode.discount, discountSource: 'promoCode' })
    }
...
Brain.prototype.verifyCustomerDiscounts = function verifyCustomerDiscounts () {
  ...
  if (_.isEmpty(discounts)) return
  const selectedDiscount = _.find(it => it.customerId === this.customer.id, discounts)
  ...
  if (!_.isNil(selectedDiscount)) {
    /*
      ...
      */
    const currentDiscount = this.tx.discount || 0
    if (selectedDiscount.discount > currentDiscount) {
      this.tx = Tx.update(this.tx, { discount: selectedDiscount.discount, discountSource: 'individualDiscount' })
    }
  }
}
