...
Brain.prototype._initBillValidatorEvents = function _initBillValidatorEvents () {
  ...
  billValidator.on('cashSlotRemoveBills', function () { self._cashSlotRemoveBills() })
  billValidator.on('startUpRemoveBills', function () { self._startUpRemoveBills() })
}
...
Brain.prototype._processReal = function _processReal (req) {
  ...
  switch (req.button) {
    ...
    case 'cashSlotBillsRemoved':
      ...
      break
    case 'startUpBillsRemoved':
      this.billValidator.startUpBillsRemoved()
        .then(done => {
          if (done) return /* TODO(siiky): idle */
        })
        .catch(/* TODO(siiky): maint */)
      break
    default:
      break
  }
...
Brain.prototype.minimumFiat = function minimumFiat () {
  ...
  const stacker = _.head(this.trader.stacker)
  return Math.max(cassette?.denomination ?? 0, stacker?.denomination ?? 0)
}
...
Brain.prototype._billsRead = function _billsRead (_data) {
  const billValidator = this.billValidator
  const data = this.isCashRecycler() ? _data : [_.set('destinationUnit', 'cashbox', _data)]
...
Brain.prototype._startUpRemoveBills = function () {
  this.browser().send({ action: 'startUpRemoveBills' })
  this.billValidator.openShutter()
}
...
Brain.prototype.updateBillsScreen = function updateBillsScreen (blockedCustomer, value) {
