...
Brain.prototype.processAction = function processAction (action, stateMachine) {
  switch (action) {
    ...
    case 'timeoutToScannerCancel':
      this.timeoutToScannerCancel()
      break
    case 'transitionScreen':
      this.transitionScreen()
      break
    case 'timeoutToFail':
      setTimeout(() => stateMachine.dispatch('FAIL'), 30000)
      break
    case 'success':
...
Brain.prototype.transitionScreen = function transitionScreen () {
  ...
  if (!appState) {
    ...
    let machineState = idCardPhoto.getState()
    switch (machineState) {
      case 'scanPhotoCard':
        appState = 'scan_photo'
        break
...
Brain.prototype.timeoutToScannerCancel = function timeoutToScannerCancel () {
  this.clearTimeoutToScannerCancel()
  this.scannerTimeout = setTimeout(() => this.scanner.cancel(), 30000)
}
...
Brain.prototype.isTierCompliant = function isTierCompliant (tier) {
  ...
  switch (tier) {
    ...
    case 'idCardPhoto':
      return !_.isEmpty(customer.idCardPhoto)
    case 'sanctions':
