...
Brain.prototype._emptyUnit = function _emptyUnit () {
  ...
  const billValidator = this.billValidator
  const billDispenser = this.billDispenser
  ...
  this._networkDown()
  return this.billValidator.emptyUnit()
    .then(r =>
      this.trader.emptyUnit(r)
        .then(() => {
          this.billValidator.updateCounts(r.units)
          if (!_.isNil(billDispenser) && billDispenser.name === 'GSR50') {
            this.billDispenser.updateCounts(r.units)
          }
        })
    )
    .catch(console.error)
...
Brain.prototype._refillUnit = function _refillUnit () {
  // Using the bill validator instance, as that is the one available 100% of the time (cash-out can be disabled, losing the bill dispenser reference)
  const billValidator = this.billValidator
  const billDispenser = this.billDispenser
  ...
  this._networkDown()
  return this.billValidator.refillUnit()
    .then(r =>
      this.trader.refillUnit(r)
        .then(() => {
          this.billValidator.updateCounts(r.units)
          if (!_.isNil(billDispenser) && billDispenser.name === 'GSR50') {
            this.billDispenser.updateCounts(r.units)
