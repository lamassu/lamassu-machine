...
Brain.prototype._registerPhone = function _registerPhone() {
  this._transitionState('registerPhone');
};
...
Brain.prototype._securityCode = function _securityCode(code) {
  ...
  this.lamassu.verifyCode(code, function(err, result) {
    if (err) return self._fiatAbortedTx();
    if (!result.success) return self._timedState('badSecurityCode');
    self._cashOut();
  });
};

Brain.prototype._cancelPhone = function _cancelPhone() {
  // TODO: extra screen + receipt if bitcoins already sent
  this._idle();
};
...
Brain.prototype._cashOut = function _cashOut() {
  ...
  if (tx.fiat > this.trader.zeroConfLimit && !this.lamassu.isSecured()) {
