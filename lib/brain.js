...
function fullDispense (bills, fiat, cassettes) {
  const len = bills.length
  let total = BN(0)

  for (let i = 0; i < len; i++) {
    total = total.add(cassettes[i].denomination.mul(bills[i].accepted))
  }
...
Brain.prototype._physicalDispense = function _physicalDispense () {
  ...
  this.billDispenser.dispense(notes, function (err, result) {
    ...
    const tx = self.tx
    let cassettes = self.trader.cassettes
    const dispenseConfirmed = fullDispense(bills, tx.fiat, cassettes)
    fillInBills(tx, bills)
    tx.error = result.err
    self.fastUpdateTx({bills: tx.bills, error: tx.error, dispenseConfirmed})
