...
Brain.prototype._processPhoneNumber = function _processPhoneNumber (number) {
  ...
  this.trader.fetchPhoneTx(number)
  .then(tx => {
    this.tx = tx
...
function fullDispense (tx) {
  console.log('DEBUG120: %j', tx.bills)
...
Brain.prototype._physicalDispense = function _physicalDispense () {
  ...
  const notes = [tx.bills[0].provisioned, tx.bills[1].provisioned]
  ...
  this.billDispenser.dispense(notes, function (err, result) {
    ...
    console.log('DEBUG121: %j', [notes, bills])
