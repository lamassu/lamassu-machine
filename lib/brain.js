...
Brain.prototype._connectedBrowser = function _connectedBrowser () {
  ...
  const fullRec = {
    ...
    cryptomatModel,
    areThereAvailableCoupons: this.trader.areThereAvailableCoupons
  }
...
Brain.prototype.smsFlowHandleReturnState = function smsFlowHandleReturnState () {
  ...
  const now = new Date()
  const isSuspended = this.customer && this.customer.suspendedUntil && new Date(this.customer.suspendedUntil) > now

  if (isStartOfTx && isSuspended) {
    const diff = new Date(this.customer.suspendedUntil).valueOf() - now.valueOf()
...
Brain.prototype._cancelPromoCode = function _cancelPromoCode () {
  if (this.tx.direction === 'cashIn') this.returnToCashInState(false)
  if (this.tx.direction === 'cashOut') this.returnToCashOutState(false)
}

Brain.prototype._submitPromoCode = function _submitPromoCode (data) {
  const promoCode = data.input

  this.trader.verifyCoupon(promoCode, this.tx).then(res => {
    this.tx = Tx.update(this.tx, { discount: res.coupon.discount })
    ...
    this.tx.direction === 'cashIn'
      ? this.returnToCashInState(true)
      : this.returnToCashOutState(true)
  }).catch(err => {
    console.log('Coupon code not found: Error ' + err)
...
Brain.prototype.returnToCashInState = function returnToCashInState (passTx) {
...
Brain.prototype.returnToCashOutState = function returnToCashOutState (passTx) {
  passTx
