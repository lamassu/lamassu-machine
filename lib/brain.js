...
Brain.prototype._processReal = function _processReal (req) {
  switch (req.button) {
    ...
    case 'badSecurityCodeOk':
      ...
      break
    // TODO: remove these events. They are handled through sms flow
...
Brain.prototype._smsSendCode = function _smsSendCode (phone, cb, time = Date.now()) {
  const self = this
  self.trader.phoneCode(phone).then(result => cb(null, result))
  .catch(err => {
    let timedOut = (Date.now() - time) > NETWORK_TIMEOUT_INTERVAL
    console.log('Error in sms send code:', err.name, err.statusCode, (!timedOut) ? '. Retrying...' : '')
    if (err.name === 'BadNumberError') return cb(err.name)
    if (!timedOut) return self._smsSendCode(phone, cb, time)
    cb('HTTPError')
  })
}

Brain.prototype.smsCompliance = function smsCompliance (returnState) {
  const self = this
  const flow = new sms.Flow()
  this.flow = flow
  ...
  flow.on('sendCode', (phone) => {
    self._smsSendCode(phone, (err, result) => {
      console.log('result from sendCode', result)
      self.tx = Tx.update(self.tx, {customerId: result.customer.id})
      console.log('added customer in transaction', result.customer.id)
      if (err) {
        if (err === 'BadNumberError') {
          return flow.handle('badPhoneNumber')
        }
        // HttpError
        self.browser().send({action: 'networkDown'})
  ...
  flow.on('success', () => {
    ...
    this.flow = null
    this.tx = Tx.update(this.tx, {phone})

    if (returnState) {
      this.smsFlowHandleReturnState(returnState)
      return
    }
...
Brain.prototype.smsFlowHandleReturnState = function smsFlowHandleReturnState (returnState) {
  if (returnState === 'acceptingFirstBill') {
    return this._idle()
  }
  ...
  if (returnState === 'chooseFiat') {
    ...
    if (this.tx.phone) {
      this._transitionState('deposit', {tx: this.tx})
...
Brain.prototype.completePayment = function completePayment (returnState) {
...
Brain.prototype._billRead = function _billRead (data) {
  ...
  if (!this.tx.phone &&
     ...
     fiat.gte(this.trader.smsVerificationThreshold)) {
    // Cancel current bill
...
Brain.prototype._registerPhone = function _registerPhone (state) {
  // this._transitionState('registerPhone', {context: this.redeem ? 'redeem' : 'deposit'})
  console.log('DEBUG: REGISTER PHONE, current state: ', state || this.state)
  this.smsCompliance(state)
}
...
Brain.prototype._processPhoneNumber = function _processPhoneNumber (number) {
  ...
  .catch(err => {
    if (err.statusCode === 404) {
      self._timedState('unknownPhoneNumber')
      throw err
    } else if (err.statusCode === 412) {
      // There are unconfirmed transactions
      this.tx = null
      return this._sendSecurityCode(number)
    } else {
      self._fiatError(err)
      throw err
    }
...
Brain.prototype._phoneNumber = function _phoneNumber (number) {
  ...
  .catch(() => {})  // Errors already handled by _processPhoneNumber
}
...
Brain.prototype._cashOut = function _cashOut () {
  ...
  if (this.exceedsZeroConf(tx) && !this.secured) {
    return this._registerPhone(this.state)
  }
