...
const Brain = function (config) {
  ...
  this.on('acceptingFirstRecyclerBills', () => this.billValidator.enable())
  this.on('acceptingRecyclerBills', () => this.billValidator.enable())
}
...
Brain.prototype._initBillValidatorEvents = function _initBillValidatorEvents () {
  ...
  billValidator.on('cashSlotRejectedBills', function () { self._cashSlotRejectedBills() })
}
 ...
 */
Brain.prototype._cashSlotRejectedBills = function () {
...
Brain.prototype._restartServices = function _restartServices (reason, restartOthers) {
  ...
  const command = this.determinePlatform === 'AAEON'
    ? 'killall chromium-browser' : 'supervisorctl restart all'
