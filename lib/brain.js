...
var Brain = function(config) {
  ...
  if (config.mockWifi) {
    this.wifi = require('./mocks/wifi')(config.wifi);
  }
  else {
    this.wifi = require('./wifi')(config.wifi);
  }
  ...
  this.pairing = require('./pairing')(config.pairing);

  config.id003.currency = this.currency;
  this.billValidator = require('id003').factory(config.id003);
  ...
  this.txLog = bunyan.createLogger({
    ...
    streams: [{
      type: 'rotating-file',
      path: this.config.transactionLogPath,
      period: '1d',
      count: 360
    }]
  });
