...
Brain.prototype._loadDb = function _loadDb () {
  try {
    var str = fs.readFileSync(this._dbPath())
    var parse = R.compose(R.map(JSON.parse), R.filter(R.isEmpty), R.split('\n'))
    var patch = parse(str)
    var dispenseTemplate = {reserved: {total: 0, sessions: {}}}
    var db = {}
...
Brain.prototype._getFiatButtonResponse = function _getFiatButtonResponse () {
  ...
  var activeDenominations = this.billDispenser.activeDenominations(txLimit,
    tx.fiat, cartridges, virtualCartridges)
...
function totalReserved (sessions) {
  return R.sum(R.pluck('amount'), sessions)
}
...
Brain.prototype._redeemDispense = function _redeemDispense () {
  ...
  var amount = tx.fiat
  var cartridges = this.trader.cartridges
  var billDispenser = this.billDispenser
  var distribution = billDispenser.constructor.billDistribution(amount, cartridges)
  var db = this.db
  ...
  distribution = billDispenser.constructor.billDistribution(amount + reserved, cartridges)
  if (!distribution && !isReserved) {
    return this._timedState('outOfCash')
  }
