...
Brain.prototype.verifyCustomerDiscounts = function verifyCustomerDiscounts () {
  const discounts = this.trader.individualDiscounts
  const customerDiscounts = _.filter(it =>
    (it.idType === 'phone' && it.value === this.customer.phone) ||
    (it.idType === 'idNumber' && it.value === this.customer.idCardDataNumber)
  , discounts)

  if (!_.isEmpty(customerDiscounts)) {
    /*
      * It's possible for a customer to have discounts associated with both phone and id card number.
      * In that case, use the bigger discount.
      */
    const selectedDiscount = _.head(_.orderBy(['discount'], ['desc'], customerDiscounts))
    this.tx = Tx.update(this.tx, { discount: selectedDiscount.discount })
  }
}
