...
Brain.prototype.smsCompliance = function smsCompliance (opts = {}) {
  ...
  flow.on('success', () => {
    ...
    .then(tx => {
      this.tx = tx
      return this.smsFlowHandleReturnState(_.assign({
        smsResult: 'success'
      }, opts))
    })
    .catch(err => {
  ...
  flow.on('fail', () => {
    ...
    if (returnState) {
      this.smsFlowHandleReturnState(_.assign({
        smsResult: 'fail'
      }, opts))
      return
    }
...
Brain.prototype.smsFlowHandleReturnState = function smsFlowHandleReturnState (opts) {
  const returnState = opts.returnState
  const result = opts.smsResult

  if (opts.redeem) {
    return this._dispenseUpdate(this.tx)
  }

  if (!returnState) return this.startScreen(true)
   ...
   */
  if (returnState === 'acceptingFirstBill' && result === 'fail') {
    return this._idle()
  }
...
Brain.prototype.startScreen = function startScreen (compliant) {
  ...
  const cashInBillExceedsThreshold = this.billValidator.lowestBill(this.tx.fiat)
  .gte(this.trader.smsVerificationThreshold) &&
  direction === 'cashIn'

  if (this.trader.smsVerificationActive &&
      cashInBillExceedsThreshold &&
      !compliant) {
