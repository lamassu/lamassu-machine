...
Brain.prototype._checkPower = function _checkPower(cb) {
  cb = cb || _.noop;
  if (!_.contains(STATIC_STATES, this.state)) return cb();

  // TODO: factor this out to a device-specific module
  var powerStatusPath = this.config.powerStatus;
  if (!powerStatusPath) return cb();
...
Brain.prototype._billValidatorErr = function _billValidatorErr(err) {
  ...
  this._checkPower(function(isPowerDown) {
