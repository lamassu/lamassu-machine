...
Brain.prototype.completeBillHandling = function completeBillHandling (returnState) {
  ...
  // Provide the reason of bill rejection. If it is not a transactionLimit, defaults to lowBalance
  const reason = this.tx.fiat.add(this.billValidator.lowestBill(BN(0))).gt(txLimit)
  ? 'transactionLimit'
  : false
  // SendOnly flag shows the user that no more bills are accepted
  // If the reason is transactionLimit or lowBalance, sendOnly is true
  // TODO: This condition is not taking into account
  //       the user's totalVolume for an existing user.
  //       It shows the lowBalance screen instead of transactionLImit
  //       Create a new brain function that handles user's totalVolume
  //       (used also in _billRead)
  const sendOnly = reason || highestBill.lte(0)
