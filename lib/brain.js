...
function chooseBillDispenser (config) {
  const billDispenser = config.billDispenser.model
  const isMockedDispenser = config.mockBillDispenser
  console.log('chooseBillDispenser', config.mockBillDispenser, config.billDispenser)

  if (isMockedDispenser) {
    return require('./mocks/billdispenser').factory(config.billDispenser)
  }
