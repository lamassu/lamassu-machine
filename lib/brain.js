...
Brain.prototype.prunePending = function prunePending (txs) {
  const isPending = tx => tx.direction === 'cashIn' &&
  tx.bills.length > 0 &&
  !tx.sendConfirmed

  const pendingTxs = _.filter(isPending, txs)

  if (_.isEmpty(pendingTxs)) return 0

  // Since it's pending we want to send and not wait for more bills
  const promises = _.map(tx => this.postTx(_.assign(tx, {send: true, timedout: true})), pendingTxs)
...
Brain.prototype.postTx = function postTx (tx) {
  const postTxF = timedout => {
    console.log('DEBUG800: %s', timedout)
  ...
  console.log('DEBUG801: %s', POLL_INTERVAL)
  const source$ = Rx.Observable.interval(POLL_INTERVAL)
  .startWith(-1)
  .combineLatest(timeout$, (x, r) => r.timedout)
  .do(r => console.log('DEBUG802: %j', r))
  .mergeMap(postTxF)
  .do(r => console.log('DEBUG803: %j', r))
  .share()
