...
Brain.prototype._physicalDispense = function _physicalDispense () {
  ...
  this.billDispenser.dispense(notes, (err, result) => {
    ...
    const consolidatedErr = (err && err.message) ||
    result.err ||
    !dispenseConfirmed && 'Partial dispense' ||
    null

    this.fastUpdateTx({bills: this.tx.bills, error: consolidatedErr, dispenseConfirmed})

    if (consolidatedErr) {
      console.log(`Dispense error: ${consolidatedErr}`)
