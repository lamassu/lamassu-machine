...
const BILL_ACCEPTING_STATES = ['billInserted', 'billRead', 'acceptingBills', 'acceptingRecyclerBills',
...
Brain.prototype._firstBill = function _firstBill () {
  this.isCashRecycler(this.billValidator)
    ? this._setState('acceptingRecyclerBills')
...
Brain.prototype._billRead = function _billRead (_data) {
  const billValidator = this.billValidator
  ...
  if (remainingFiatToInsert.lt(currentBills)) {
    ...
    if (highestBill.lte(0)) {
      ...
      return this._billReadResolver({ success: false })
    }
  ...
  if (!isCompliant) {
    ...
    if (idTier) return this._billReadResolver({ success: false, callback: this.transitionToVerificationScreen, args: [nonCompliantTier] })

    return this._billReadResolver({ success: false, callback: this.runComplianceTiers, args: [nonCompliantTiers] })
  }

  // verify if any non compliant manual trigger was scanned and end tx
  if (this.txBlockedByManualTrigger) {
    this.txBlockedByManualTrigger = false
    return this._billReadResolver({ success: false, callback: this.showBlockedCustomer, args: [] })
  }

  if (this.isCashRecycler(billValidator)) {
  ...
  return this._billReadResolver({ success: true })
}

Brain.prototype._billReadResolver = function _billReadResolver ({ success, callback, args }) {
  if (success) {
    return this.isCashRecycler(this.billValidator) ? this.recyclerBillFinish(this.bills) : null
  }
...
Brain.prototype._billStandby = function _billStandby () {
  if (this.state === 'acceptingRecyclerBills' || this.state === 'acceptingBills' || this.state === 'acceptingFirstBill') {
    this.enableBillValidator()
  }
}
...
/**
 * Clear the rejected bills and keep i'ts amount as the lastRejectedBillFiats
 *
 * @name clearBills
 * @function
 *
...
Brain.prototype.insertBills = function insertBills (bills) {
  ...
  if (!serverVersion || semver.lt(serverVersion, '7.5.1-beta.0')) {
    ...
    this.bills = _.map(it => Tx.createBillDeprecated(it.denomination, exchangeRate, this.tx), bills)
  } else {
...
Brain.prototype.recyclerBillFinish = function recyclerBillFinish (bills, blockedCustomer) {
  return this.fastUpdateTx(Tx.billUpdate(bills))
    .then(() => this.completeBillHandling(blockedCustomer))
...
Brain.prototype.recyclerContinue = function recyclerContinue () {
  return this.billValidator.cashCount()
    .then(bills => Promise.all([bills, this._billRead(bills)]))
}
