...
const BILL_ACCEPTING_STATES = ['billInserted', 'billRead', 'acceptingRecyclerBills', 'acceptingBills',
...
Brain.prototype._idleTwoWay = function _idleTwoWay (localeInfo) {
  ...
  return this.billDispenser.init({
    ...
    .then(() => {
      return this._chooseCoinScreen(localeInfo, uiCassettes)
    })
    .catch(console.log)
}
...
Brain.prototype._billRead = function _billRead (_data) {
  ...
  if (!isCompliant) {
    ...
    if (idTier) return { success: false, callback: this.transitionToVerificationScreen, args: [nonCompliantTier] }
...
Brain.prototype._uiCredit = function _uiCredit () {
  const tx = Tx.update(this.tx, Tx.billUpdate(this.bills))
...
/**
 * Clear the rejected bills and keep it's
 * amount as the lastRejectedBillsFiat
 *
 * @name clearBills
 * @function
 *
