...
Brain.prototype._start = function _start () {
  ...
  const printPaperWallet = deviceConfig.compliance.paperWallet &&
  ...
  if (printPaperWallet) {
    ...
    kioskPrinter.checkStatus(deviceConfig.kioskPrinter)
      .then(() => {
        const wallet = this.createBitcoinWallet()
        const printerCfg = deviceConfig.kioskPrinter
        console.log(wallet)
        kioskPrinter.printWallet(wallet, printerCfg)
          .then(() => {
            this.tx = Tx.update(this.tx, {isPaperWallet: true})
            this._handleScan(wallet.publicAddr)
          })
          .catch((err) => {
            console.log('[ERROR]: Failed to print the paper wallet.', err)
            this._idle()
          })
      })
      .catch((err) => {
...
Brain.prototype.createBitcoinWallet = function () {
    const keyPair = bitcoin.ECPair.makeRandom()
    const segwitAddr = bitcoin.payments.p2wpkh({ pubkey: keyPair.publicKey })

    return {
        publicAddr: segwitAddr.address,
        privateAddr: keyPair.privateKey.toString('hex')
    }
}
