...
function txMerger (objValue, srcValue, key, object, source, stack) {
  ...
  if (key === 'fiat') return srcValue.add(objValue)
  if (key === 'cryptoAtoms') return truncateCrypto(srcValue.add(objValue))
}
...
Brain.prototype._billValid = function _billValid () {
  ...
  const totalFiat = this.tx.fiat.add(pending.fiat)
  const fiatBalance = this.balance().sub(totalFiat)
...
function pendingTx (pending, tx) {
  return _.mergeWith(txMerger, pending, tx)
}

Brain.prototype._uiCredit = function _uiCredit () {
  const tx = pendingTx(this.pending, this.tx)
...
function truncateCrypto (cryptoAtoms) {
