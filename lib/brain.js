...
Brain.prototype.smsFlowHandleReturnState = function smsFlowHandleReturnState () {
  ...
  const cashInModifier = () => this.billValidator.lowestBill(coin.minimumTx)
  const cashOutModifier = () => this.minimumFiat()
  const simpleAmount = this.complianceAmount()

  const modifier = this.tx === 'cashIn' ? cashInModifier() : cashOutModifier()
  const amount = isStartOfTx ? simpleAmount.add(modifier) : simpleAmount
