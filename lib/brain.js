...
Brain.prototype.waitForCollection = function waitForCollection (dispenseGenerator) {
  const txId = this.tx.id
  let g = dispenseGenerator // change execution context with .call
...
Brain.prototype._dispenseGenerator = function* dispenseGenerator(notes, txId) { // TODO move to separate function
  let batchNum = 1
  let result = []
  for (let index = 0; index < notes.length; index++) { // cannot yield inside forEach
    let yielded = yield this.billDispenser.dispense(notes[index])
    if (yielded !== undefined) result.push(yielded)
    batchNum = index + 1
    this.browser().send({dispenseBatch: {current: batchNum, of: notes.length} })
  }
...
Brain.prototype._physicalDispense = function _physicalDispense () {
  ...
  this.browser().send({dispenseBatch: {current: 1, of: notesToDispense.length} })
  this._transitionState('dispensing')
  emit('billDispenserDispensing')

  this.waitForCollection(this._dispenseGenerator(notesToDispense, txId))
}
