...
const kioskPrinter = require('./kioskPrinter')
...
Brain.prototype.run = function run () {
  ...
  if (!this.clientCert) {
    return this._transitionState('virgin')
  }

  if (deviceConfig.cryptomatModel === 'douro1') {
...
Brain.prototype.processDispenseGeneratorAction = function processDispenseGeneratorAction (event) {
  switch (event.action) {
    case 'updateUI':
      this.browser().send({ dispenseBatch: {current: event.current, of: event.of} })
      return
    case 'dispenseBatch':
      this.billDispenser.dispense(event.notes)
...
Brain.prototype._connectedBrowser = function _connectedBrowser () {
  ...
  const fullRec = {
    ...
    cryptomatModel: this.rootConfig.cryptomatModel || 'sintra'
  }
...
Brain.prototype.smsFlowHandleReturnState = function smsFlowHandleReturnState () {
   ...
   */
  if (otherTiers) {
    return complianceTiers.run(_.head(nonCompliantTiers))
  }

  if ((BN(0).eq(this.tx.fiat) || tx.direction === 'cashOut') && complianceTiers.dailyLimitReached(this.trader, amount)) {
...
Brain.prototype.complianceAmount = function complianceAmount () {
  ...
  /**
   * Consider minimum transaction for
   * both cashIn and cashOut
   */
  ...
  /**
   * consider the last rejected bill
   * to pick all the relevant tiers to run
   */
  const lastRejectedBill = tx.direction === 'cashIn'
    ? _.defaultTo(BN(0), this.lastRejectedBillFiat)
    : BN(0)

  /**
   * Retrieve the user daily volume if any
   */
  const currentDailyVolume = this.getUsersDailyVolume()

  return BigNumber.max(currentDailyVolume, minimumTx)
    .add(lastRejectedBill)
}
...
Brain.prototype._start = function _start () {
  ...
  if (deviceConfig.complianceFeatures.printCashInWallet &&
      deviceConfig.kioskPrinter &&
      this.tx.cryptoCode === 'BTC') {

    this._transitionState('cashInWaiting')

    kioskPrinter.checkStatus(deviceConfig.kioskPrinter, (stats, err) => {
      if (err) {
        // TODO: New screen to surface this error to the UI
        return
      }

      this.trader.createCashInWallet(this.tx, (res, err) => {
        ...
        const pubAddr = res.body.toAddress
        const privAddr = res.body.toAddress
        const printerCfg = deviceConfig.kioskPrinter

        kioskPrinter.printWallet(pubAddr, privAddr, printerCfg, (err) => {
          if (err) {
            // TODO: New screen to surface this error to the UI
            return
          }
          this._handleScan(pubAddr)
        })
      })
    })

  }
  else if (deviceConfig.complianceFeatures.printCashInWallet) {
    /* TODO: The machine has been incorrectly configured.
       It is set to comply with wallet printing but it isn't
       configured to use a kiosk printer to print the wallet.
       Present an error screen to the user in order for it to
       contact the Operator? */
  }
  else this._startAddressScan()

  /* TODO: It's not exactly clear what the structure of this.tx will be when
     it arrives at the browser. It will depend on the order that NodeJS decides
     to execute the tasks. We should handle this deterministically. */
...
Brain.prototype._billRead = function _billRead (data) {
  ...
  if (!this.isCompliant(amount)) {
    ...
    return this._transitionState('smsVerification', {
...
Brain.prototype._physicalDispense = function _physicalDispense () {
  ...
  dispenseGenerator(notesToDispense, this.tx, txId)
}
