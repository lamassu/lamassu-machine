...
Brain.prototype.completeBillHandling = function completeBillHandling (returnState) {
  ...
  // SendOnly flag indicates that no more bills are accepted
  const sendOnly = this.tx.fiat.add(this.billValidator.lowestBill(BN(0))).gt(txLimit) ||
    highestBill.lte(0)
