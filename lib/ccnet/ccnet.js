...
Ccnet.prototype.hasShutter = () => false
...
Ccnet.prototype.run = function (cb, _) {
  return rs232.create(this.device)
    .then(() => {
      ...
      statesFsm.on('billsAccepted', () => this.emit('billsAccepted'))
      statesFsm.on('jam', () => this.emit('jam'))
      statesFsm.on('billsValid', () => this.emit('billsValid'))
      statesFsm.on('billsRejected', () => this.emit('billsRejected'))
      statesFsm.on('stackerOpen', () => this.emit('stackerOpen'))
      statesFsm.on('billsRead', (data) => {
