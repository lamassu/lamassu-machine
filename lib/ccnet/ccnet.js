...
Ccnet.prototype.enable = function () {
  ...
  const command = Buffer.concat([Buffer.from(commands.ENABLE_BILL_TYPES), billsBuffer])
  this._send(command)
}

Ccnet.prototype.disable = function () {
  const command = Buffer.concat([Buffer.from(commands.ENABLE_BILL_TYPES), Buffer.alloc(6)])
  this._send(command)
}
