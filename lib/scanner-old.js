...
const delayedshot = (mode, delay) => new Promise((resolve, reject) => {
  if (!openedFromOutside && opened) {
    console.log("Camera is already open. Shouldn't happen.")
    return reject(new Error('Camera open'))
  }

  const { cam } = _prepareForCapture(mode)

  const capture = () => {
    if (cancelFlag) return reject(new Error('Camera canceled'))
    cam.capture(success => success ?
      resolve(Buffer.from(cam.frameRaw())) :
...
function scan (mode, resultCallback, captureCallback) {
  ...
  let handle = setInterval(capture, 100)
...
function scanPDF417 (callback, idCardStillsCallback) {
  ...
  scan('photoId', callback, function (modeConfig, width, height, frame, greyscale, _callback) {
    ...
    const result = manatee.scanPDF417(greyscale, width, height)
    if (!result) {
...
function scanPairingCode (_shouldSaveQrAttempts, callback) {
  scanQR((err, res) => {
...
function scanMainQR (cryptoCode, _shouldSaveQrAttempts, callback) {
...
function scanPK (_shouldSaveQrAttempts, callback) {
  scanQR(function (err, result) {
    if (err) return callback(err)
    if (!result) return callback(null, null)
