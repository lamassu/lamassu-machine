...
function checkStatus (printerCfg) {
  return new Promise((resolve, reject) => {
    const model = checkPrinterModel()
    if (model === printerModel.zebra)
      return checkStatusZebra(printerCfg)
        .then(() => resolve())
        .catch((err) => reject(err))
    reject(errorMsgUnsupportedPrinter())
  })
}
...
function openSerialPort(rs232Settings) {
  return new Promise((resolve, reject) => {
...
function checkStatusZebra (printerCfg) {
  return new Promise((resolve, reject) => {
    return openSerialPort(zebraRS232Settings)
      .then((port) => {
