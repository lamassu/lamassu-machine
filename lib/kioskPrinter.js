...
function translateZebraStatusCode (statusCode) {
  ...
  humanReadableStatus.hasErrors = statusCode.hasErrorFlag === '1' ? true : false
  humanReadableStatus.hasWarnings = statusCode.hasWarningFlag === '1' ? true : false

  errorsList.forEach(entry => {
      const code = parseInt(statusCode.errorCodes[entry.index])
      if (code & entry.code)
        humanReadableStatus.errorList.push(entry)
  })
  warningsList.forEach(entry => {
      const code = parseInt(statusCode.warningCodes[entry.index])
      if (code & entry.code)
        humanReadableStatus.warningsList.push(entry)
  })
