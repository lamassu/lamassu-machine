...
const LOG_STDERR = require('minimist')(process.argv.slice(2)).devBoard
...
const exitCode2str = code => EXIT_CODES[code] || code
...
const opts2array = _.flow(
  _.toPairs,
  _.filter(([_, v]) => v !== undefined),
  _.flatMap(([k, v]) => [`-${k}`, v]),
)

const spawn = (pos, opts) => {
  const args = _.concat(pos, opts2array(opts))
...
const spawn_zxing = (dev, opts) => spawn([dev, 'zxing'], opts)
const spawn_supyo = (dev, opts) => spawn([dev, 'supyo'], opts)
const spawn_camcheck = dev => spawn([dev, 'camcheck'], {})


const scanQR = (dev, pickFormat, fps, rotate) =>
  set_event_handlers(
    spawn_zxing(dev, { fps, format: 'QRCode', rotate }),
    pickFormat
  )

const scanPDF417 = (dev, pickFormat, fps, tmpdir, rotate) =>
  set_event_handlers(
    spawn_zxing(dev, { fps, format: 'PDF417', tmpdir, rotate }),
    pickFormat
  )

const detectFace = (dev, pickFormat, fps, minsize, cutoff, rotate) =>
  set_event_handlers(
    spawn_supyo(dev, { fps, minsize, cutoff, rotate }),
    pickFormat
  )
