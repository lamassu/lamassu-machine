...
const set_event_handlers = ({ ac, ko }, pickFormat) =>
  [
    ...
    new Promise((resolve, reject) => {
      ...
      console.debug("camera streamer started")
      ko.on('error', reject)
      // TODO: is this useful?
      ...
      if (LOG_STDERR)
        readline
          ...
          .on('line', line => console.debug("camera-streamer:", line))
...
const spawn_zxing = (dev, fmt, tmpdir) =>
  spawn(tmpdir ? [dev, 'zxing', fmt, tmpdir] : [dev, 'zxing', fmt])
...
const scanPDF417 = (dev, pickFormat, tmpdir) =>
  set_event_handlers(spawn_zxing(dev, 'PDF417', tmpdir), pickFormat)
