...
const set_event_handlers = (ko, pickFormat) => {
  ...
  const promise = new Promise((resolve, reject) => {
    ...
    if (LOG_STDERR) {
      rl = readline.createInterface({ input: ko.stderr, terminal: false, historySize: 0 })
      rl.on('line', line => console.log("camera-streamer:", line))
    }
  ...
  return [korose, promise]
}


const spawn = args =>
  execFile(CAMERA_STREAMER, args, { encoding: 'buffer' })
