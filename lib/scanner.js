...
function camOn() {
  ...
  cam.controlSet(0x9a0902, config.lowExposure);    // Set absolute exposure
...
function alternateExposure() {
  var exposure = lowExposure ? config.highExposure : config.lowExposure;
...
function scan(resultCallback, captureCallback, alternateExposureFlag) {
  ...
  lowExposure = true;
  cam.controlSet(0x9a0902, config.lowExposure);
  ...
  function capture(callback) {
    cam.capture(function (success) {
      ...
      if (alternateExposureFlag) {
        var now = Date.now();
        if (now - lastExposure > config.exposureAdjustTime) alternateExposure();
        lastExposure = now;
      }
