...
// TODO: Make camOn, camOff, config private, just use scan* functions and cancel
// This lets us control parameters separately.
function camOn(_mode) {
  mode = _mode
...
function alternateExposure() {
  var exposure = lowExposure ?
    configuration.highExposure :
    configuration.lowExposure;
...
function scan(resultCallback, captureCallback, alternateExposureFlag) {
  ...
  var processing = true;
  lowExposure = true;
  var _alternateExposureFlag = configuration.lowExposure && alternateExposureFlag;
