...
function scan (mode, resultCallback, captureCallback) {
  ...
  const capture = () => cameraStreamer.captureFrame(getCameraDevice(mode), pickFormat(mode))
    .then(frame => jpg.decompressSync(frame, { format: jpg.FORMAT_GRAY }))
    .then(greyscale => {
      if (cancelFlag) return shutdown()
  ...
  opened = true
  handle = setInterval(capture, 100)
}
