...
function camOn() {
  if (cam) throw new Error('Camera is already on.');
  ...
  var contrast = configuration.contrast;
  cam.controlSet(contrastControl.id, contrast);

  cam.controlSet(0x9a0901, 1);      // Set exposure to manual
  cam.controlSet(0x9a0902, configuration.lowExposure);    // Set absolute exposure
...
function camOff(callback) {
  cam.stop(function() {
    cam.controlSet(0x9a0902, configuration.lowExposure);
    cam = null;
    imageBuffer = null;
...
function scan(resultCallback, captureCallback, alternateExposureFlag) {
  ...
  setTimeout(function() {
    camOff(function() {
      resultCallback(new Error('Scanner timeout'));
    });
  }, 5000);
...
function scanQR(callback) {
  scan(callback, function(_callback) {
    imageBuffer = cam.toGrey(imageBuffer);
