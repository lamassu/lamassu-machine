...
function setConfig (cam) {
  var format = null

  const pixelRes = format => format.width * format.height
  const sortByPixelRes = _.sortBy(pixelRes)
  const isSuitableRes = currentRes => {
    const isUnderMaxAcceptableResolutions = _.map(res => pixelRes(currentRes) < pixelRes(res), maxCamResolutions).every(bool => bool === true)
    const isAboveMinAcceptableResolutions = _.map(res => pixelRes(currentRes) > pixelRes(res), minCamResolutions).every(bool => bool === true)
    return isUnderMaxAcceptableResolutions && isAboveMinAcceptableResolutions
  }

  const availableFormats = sortByPixelRes(cam.formats.filter(f => f.formatName === 'MJPG'))

  _.map(elem => {
    if (isSuitableRes(elem)) format = elem
  }, availableFormats)
