...
const diagnosticPhotos = () => {
  ...
  const promises = [
    delayedshot('', 1, '/dev/video-scan'),
    delayedshot('', 1, '/dev/video-front')
  ]
  return Promise.allSettled(promises)
    .then(([scan, front]) => {
      cameraStreamer.setVerbose(IS_VERBOSE)
      return {
        scan: scan.status === 'fulfilled' && scan.value ? scan.value.toString('base64') : null,
        front: front.status === 'fulfilled' && front.value ? front.value.toString('base64') : null
      }
    })
}
