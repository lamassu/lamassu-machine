...
function scanPDF417 (callback, idCardStillsCallback) {
  let captureIdCardPhoto = false
  let idCardTimer = 4000
  let endInterval = 24000
  let handleIdCardPhoto = setInterval(() => {
    captureIdCardPhoto = true
    idCardTimer = idCardTimer + 4000
  }, idCardTimer)

  scan('photoId', callback, function (modeConfig, width, height, frame, greyscale, _callback) {
    if (idCardTimer > endInterval) {
      captureIdCardPhoto = false
      clearInterval(handleIdCardPhoto)
    }
    var result = manatee.scanPDF417(greyscale, width, height)
    if (!result) {
      if (captureIdCardPhoto) idCardStillsCallback(frame)
      captureIdCardPhoto = false
