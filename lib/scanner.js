...
function scanQR (callback) {
  scan('qr', callback, function (modeConfig, width, height, frame, greyscale, _callback) {
    ...
    const dirname = '/tmp/scan-attempts/'
    if (!fs.existsSync(dirname)) {
      console.log('QR CODE SCAN DEBUG: Created a temporary folder for scanning attempts in: ' + dirname)
      fs.mkdirSync(dirname)
    }
    const timestamp = new Date().valueOf()
    const decodedImageData = Buffer.from(frame, 'base64')
    const filename = dirname + timestamp + '.jpg'
    console.log('QR CODE SCAN DEBUG: New scanning attempt saved in ' + filename)
    fs.writeFileSync(filename, decodedImageData)
...
function _prepareForCapture (mode) {
  ...
  const lsusb = cp.execSync('lsusb').toString()
  const devrules = cp.execSync(`udevadm info -a --name ${device}`).toString()
  console.log(`QR CODE SCAN DEBUG:\n\n***********************\n\nUSB Devices List (Bus ID, Device ID, USB ID and Title)\n\n${lsusb}\n\n***********************`)
  console.log(`QR CODE SCAN DEBUG:\n\n***********************\n\nDEV RULES ${device}\n\n${devrules}\n\n***********************`)
