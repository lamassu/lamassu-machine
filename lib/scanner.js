...
function config (_configuration) {
  ...
  rotate = configuration.rotate === true
}
...
const scanQR = callback => {
  const [korose, promise] = cameraStreamer.scanQR(getCameraDevice('qr'), pickFormat('qr'), DEFAULT_FPS, rotate)
...
const scanPDF417 = (callback, idCardStillsCallback) => {
  ...
  fs.mkdtemp(path.join(os.tmpdir(), 'failed-scans-'))
    ...
    .then(tmpdir => {
      ...
      const [korose, promise] = cameraStreamer.scanPDF417(device, pickfmt, DEFAULT_FPS, tmpdir, rotate)
      replace_kogoroshiya(korose)
...
const detectFace = (mode, minsizeDef, cutoffDef, callback) => {
  ...
  const [korose, promise] = cameraStreamer.detectFace(device, pickFormat(mode), DEFAULT_FPS, minsize, cutoff, rotate)
  replace_kogoroshiya(korose)
  promise
    .then(frame => {
      clear_kogoroshiya()
...
const scanPairingCode = callback =>
  scanQR(outCallback2inCallback(callback))
