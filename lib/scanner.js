...
let DEFAULT_FPS = 10
const DEFAULT_DIAGNOSTIC_DELAY = 3
let DIAGNOSTIC_DELAY = DEFAULT_DIAGNOSTIC_DELAY
...
function config (_configuration) {
  const getConfDelay = camera => _.defaultTo(DEFAULT_DIAGNOSTIC_DELAY, _.get([camera, 'diagnosticDelay'], configuration))
  configuration = _configuration
  DIAGNOSTIC_DELAY = Math.max(getConfDelay('scanner'), getConfDelay('frontFacingCamera'))
}
...
const diagnosticPhotos = () => {
  ...
  return delayedshot('', DIAGNOSTIC_DELAY, '/dev/video-scan')
    .then((scan) => {
      if (scan) {
    ...
    .catch(() => {})
    .then(() => delayedshot('', DIAGNOSTIC_DELAY, '/dev/video-front'))
    .then((front) => {
      if (front) {
