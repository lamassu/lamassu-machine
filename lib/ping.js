...
const PACKAGES = [
  {
    url: `http://archive.ubuntu.com/ubuntu/pool/universe/p/python-defaults/python-defaults_2.7.18-2.tar.gz`,
    size: 8900
  }
]
...
const checkDownloadSpeed = () => {
  const promises = _.map(package => {
    return testConnection.checkDownloadSpeed(package.url, package.size)
      .then(speed => ({ url: package.url, 'MB/s': speed.mbps * 0.125 }))
  }, PACKAGES)
