...
Id003Rs232.prototype.open = function open(cb) {
  ...
  if ('string' === typeof(devices)) return this._open(devices, cb);
  ...
  sp.list(function (err, ports) {
    ...
    devices.some(function (device) {
      ports.some(function (port) {
        if (device === port.comName) return (openDevice = device);
        return false;
      });
    });
    if (openDevice) return self._open(openDevice, cb);
...
Id003Rs232.prototype.send = function send(command) {
  ...
  this.serial.write(Buffer.concat([buf, crcBuf], length));
};
