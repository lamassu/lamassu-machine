...
PuloonRs232.prototype.reset = function reset (cartridges, fiatCode, cb) {
  ...
  this._send(buildFrame(0x44, []), 'reset', function () {
    ...
    self._getSerialNumber(function (err, serialNumber) {
      ...
      self._setBillLengths(cartridges, fiatCode, cb)
    })
  })
}
...
function billLengthData (cartridges, fiatCode) {
  ...
  for (var i = 0; i < numCartridges; i++) {
    ...
    if (!billLength) {
      throw new Error('Unsupported denomination: ' + cartridge.denomination + ' for fiatCode: ' + fiatCode)
    }
...
fiatCode, cb) {
  var data = billLengthData(cartridges, fiatCode)
