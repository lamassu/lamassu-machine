...
PuloonRs232.prototype.open = function open(callback) {
  console.log('DEBUG device: ' + this.device); // DEBUG
...
function billLengthData(cartridges, currency) {
  ...
  for (var i = 0; i < numCartridges; i++) {
    ...
    if (!billLength) throw new Error('Unsupported denomination: ' +
      cartridge.denomination + ' for currency: ' + currency);
...
PuloonRs232.prototype._setBillLengths = function _setBillLengths(cartridges, currency, cb) {
