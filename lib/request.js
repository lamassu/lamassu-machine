...
module.exports = function request(opts) {
  ...
  var cancel = false;
  var requestId = wreckOpts.headers['request-id'];

  polls[requestId] = function() { cancel = true; };
    ...
    .catch(function(err) {
      if (err instanceof RetriableError) {
        ...
        return P.delay(retryDelay).then(function() {
          if (cancel) throw new CancelError();
  ...
  return retriedRequest()
  .finally(function() {
    delete polls[requestId];
  });
};

module.exports.reset = function reset() {
  _.forEach(polls, function(poll) { poll(); });
  polls = {};
};
