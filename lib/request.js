...
function request (configVersion, globalOptions, options) {
  ...
  const gotOptions = {
    ...
    isStream: _.defaultTo(false, options.isStream),
    path: options.path,
    body: options.body,
    retries,
    timeout: 10000,
  ...
  console.log(gotOptions.isStream, options.path, gotOptions.json, 'APOWDJKAPWOIDJUAWPIODJAWPODIJAWPOIAJWDPOIAWJDPOAIJ')
