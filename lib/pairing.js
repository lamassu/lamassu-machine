...
Pairing.prototype.pair = function pair(data, cb) {
  ...
  jsonquest(options, function (err, res, body) {
    ...
    fs.writeFile(self.config.connectionInfoPath, JSON.stringify(connectionInfo), function (err) {
...
Pairing.prototype.init = function init(cb) {
  ...
  var basePath = path.dirname(this.config.certPath);
  try { fs.mkdirSync(basePath); } catch (ex) {}
  selfSign.generateCertificate(function(err, certPem, keyPem) {
    if (err) return cb(err);
    try {
      fs.writeFileSync(config.certPath, certPem);
      fs.writeFileSync(config.keyPath, keyPem);
    } catch (ex) {
      console.log(ex);
      return cb(ex);
    }
