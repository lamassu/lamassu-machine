...
function pair (totemStr, clientCert, connectionInfoPath, model, numOfCassettes, numOfRecyclers) {
  ...
  return got(`https://${hostname}:${PORT}/ca?token=${tokenHash}`, initialOptions)
    .then(r => {
      ...
      const query = querystring.stringify({token, model, numOfCassettes, numOfRecyclers})
