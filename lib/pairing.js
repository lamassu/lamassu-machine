...
Pairing.prototype.pair = function pair (data, cb) {
  ...
  jsonquest(options, function (err, res, body) {
    ...
    if (body && body.err) return cb(new Error(body.err))

    var deviceId = res.peerDeviceId
    console.log('DEBUG15: %j', deviceId)

    if (deviceId !== connectionInfo.deviceId) {
...
Pairing.prototype._fetchConnectionInfo = function _fetchConnectionInfo () {
  try {
    var connectionInfo = JSON.parse(fs.readFileSync(this.config.connectionInfoPath))

    // backwards compat
    connectionInfo.deviceId = connectionInfo.deviceId || connectionInfo.fingerprint
    connectionInfo.fingerprint = undefined
