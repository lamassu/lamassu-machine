...
// Exported for /tools/decode-pairing-totem.js
const extract = totem => {
  totem = Buffer.from(bsAlpha.decode(totem))
  ...
  return { totem, hostname, expectedCaHash, token, tokenHash }
}

function pair (totemStr, clientCert, connectionInfoPath, numOfCassettes, numOfRecyclers) {
  const { totem, hostname, expectedCaHash, token, tokenHash } = extract(totemStr)
  ...
  return got(`https://${hostname}:${PORT}/ca?token=${tokenHash}`, initialOptions)
    .then(r => {
      ...
      const query = querystring.stringify({token, numOfCassettes, numOfRecyclers})
...
module.exports = { init, pair, unpair, isPaired, connectionInfo, getCert, extract }
