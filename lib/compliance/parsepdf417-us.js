...
const states = {
  ...
  'USA': ['AL', 'AK', 'AS', 'AZ', 'AR', 'CA', 'CO', 'CT', 'DE', 'DC',
...
function normalize (result, version, issuingAuthorityNumber) {
  ...
  result.country = result.country || getCountry(result.state)

  if (result.country === 'USA' && version > 1) {
    result.dateOfBirth = date.substr(4, 4) + date.substr(0, 4)
    result.expirationDay = expirationDate ? expirationDate.substr(2, 2) : '00'
    result.expirationMonth = expirationDate ? expirationDate.substr(0, 2) : '00'
    result.expirationYear = expirationDate ? expirationDate.substr(4, 4) : '00'
    result.expirationDate = expirationDate ? expirationDate.substr(4, 4) + expirationDate.substr(0, 4) : expirationDate
  } else if (result.country !== 'USA' || version === 1) {
    result.expirationDay = expirationDate ? expirationDate.substr(6, 2) : '00'
    result.expirationMonth = expirationDate ? expirationDate.substr(4, 2) : '00'
    result.expirationYear = expirationDate ? expirationDate.substr(0, 4) : '00'
  }
