...
function consecutiveDays (trigger, txHistory) {
  const history = _.concat(txHistory)({ created: new Date()})
  const { thresholdDays } = trigger

  const dailyQuantity = _.compose(_.countBy(daysSince), _.map('created'), filterNoFiatTx)(history)
  const hasPassed = _.every(it => dailyQuantity[it])(_.range(0, thresholdDays))
...
function getAmountToHardLimit (triggers, history, tx) {
  const filterByHardLimit = _.filter(it =>
    it.requirement === REQUIREMENTS.BLOCK || it.requirement === REQUIREMENTS.SUSPEND
  )

  const filterByDirection = _.filter(it =>
    tx.direction !== it.direction && it.direction !== DIRECTIONS.BOTH
  )
  ...
  return BN(_.min(minAmount, minVolume) || Infinity)
}
