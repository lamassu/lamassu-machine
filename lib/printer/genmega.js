const { rpu, return_codes } = require('genmega')
...
const runWithOpenPort = (printerCfg, func) =>
  ...
  new Promise((resolve, reject) => {
    const { iRet } = rpu.open(printerCfg.address)
    if (iRet < 0) return reject(new Error(return_codes[iRet.toString()]))
...
function checkStatus (printerCfg) {
  return runWithOpenPort(printerCfg, () => {
    const { iRet, result } = rpu.status()
    if (iRet < 0) return Promise.reject(new Error(return_codes[iRet.toString()]))
...
function printReceipt (data, printerCfg, receiptConfig) {
  return runWithOpenPort(printerCfg, () => {
    ...
    const { iRet: iRetPrintText } = rpu.printText(buffer)
    if (iRetPrintText < 0) return Promise.reject(new Error(return_codes[iRetPrintText.toString()]))

    const { iRet: iRetCutPaper } = rpu.cutPaper()
    if (iRetCutPaper < 0) return Promise.reject(new Error(return_codes[iRetCutPaper.toString()]))
    return Promise.resolve()
  })
}
