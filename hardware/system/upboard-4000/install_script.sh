...
# device config
cp hardware/codebase/upboard-4000/$MACHINE/device_config.json ./
...
sudo cp -r /opt/lamassu-machine/hardware/system/upboard-4000/$MACHINE/supervisor/conf.d/ /etc/supervisor/
sudo sed -i 's/user=machine/user=ubilinux/g' /etc/supervisor/conf.d/lamassu-browser.conf

# Udev config files
sudo cp -r /opt/lamassu-machine/hardware/system/upboard-4000/$MACHINE/udev/* /etc/udev/rules.d/
