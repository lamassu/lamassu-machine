...
function startPage(text) {
  ...
  updateButtonStyles();
  setTimeout(function () {
    var div = document.getElementById('js-terms-text-div').offsetHeight;
...
function setPrimaryLocales(primaryLocales) {
  ...
  for (var i = 0; i < sortedPrimaryLocales.length; i++) {
    ...
    var li = nativeName === englishName ? '<li class="square-button" data-locale="' + l + '">' + englishName + '</li>' : '<li class="square-button" data-locale="' + l + '">' + nativeName + '<span class="english">' + englishName + '</span> </li>';
    languages.append(li);
  }
...
function setExchangeRate(_rates) {
  ...
  if (rates.cashOut) {
    ...
    );var localizedCashOutCryptoToFiat = locale.translate('1 %s = %s %s').fetch(cryptoCode, cashOutCryptoToFiat, fiatCode);
...
function highBill(highestBill, reason) {
  var reasonText = reason === 'transactionLimit' ? locale.translate('Transaction limit reached.').fetch() : locale.translate("We're a little low.").fetch();
